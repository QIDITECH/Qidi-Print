...
from QD.Application import Application #To modify the maximum zoom level.
from QD.i18n import i18nCatalog
from QD.Scene.Platform import Platform

from QD.Scene.PlatformForOpen import PlatformForOpen

from QD.Scene.PlatformForDelete import PlatformForDelete
from QD.Scene.PlatformForFlat import PlatformForFlat

from QD.Scene.PlatformForCopy import PlatformForCopy
from QD.Scene.PlatformForArrange import PlatformForArrange

# from QD.Scene.PlatformForButton import PlatformForButton
# from QD.Scene.Platform7 import Platform7
...
class BuildVolume(SceneNode):
    ...
    def __init__(self, application: "QIDIApplication", parent: Optional[SceneNode] = None) -> None:
        ...
        # _theme = self._application.getTheme()
        # self._press_axis_color =Color(*_theme.getColor("press_axis").getRgb()) 
        # self._updateColors()
        ...
        self._button_background_open = None

        self._button_background_delete = None
        self._button_background_flat = None
        self._button_background_copy = None
        self._button_background_arrange = None
        ...
        self._platform_open = PlatformForOpen(self)
        self._platform_delete = PlatformForDelete(self)
        self._platform_falt = PlatformForFlat(self)
        self._platform_copy = PlatformForCopy(self)
        self._platform_arrange = PlatformForArrange(self)
        self.offset_Vector = Vector(0, 0, 0)
        # self._platformbutton = PlatformForButton(self)

        # self._platform = Platform(self)

        # self._platform7 = Platform7(self)
        # self._platform6.setActiveAxis()
    ...
    def render(self, renderer):
        ...
        if not self._shader:
            ...
            self._grid_shader.setUniformValue("u_gridColor0", Color(*theme.getColor("gray_1").getRgb()))
            self._grid_shader.setUniformValue("u_gridColor1", Color(*theme.getColor("gray_2").getRgb()))
            self._button_mesh_open = OpenGL.getInstance().createShaderProgram(Resources.getPath(Resources.Shaders, "color.shader"))

            self._button_mesh_delete = OpenGL.getInstance().createShaderProgram(Resources.getPath(Resources.Shaders, "color.shader"))
            self._button_mesh_flat = OpenGL.getInstance().createShaderProgram(Resources.getPath(Resources.Shaders, "color.shader"))
            self._button_mesh_copy = OpenGL.getInstance().createShaderProgram(Resources.getPath(Resources.Shaders, "color.shader"))
            self._button_mesh_arrange = OpenGL.getInstance().createShaderProgram(Resources.getPath(Resources.Shaders, "color.shader"))


        renderer.queueNode(self, mode = RenderBatch.RenderMode.Lines)
        renderer.queueNode(self, mesh = self._origin_mesh, backface_cull = True)
        renderer.queueNode(self, mesh = self._button_background_arrange, shader = self._button_mesh_arrange, backface_cull = True,sort = -10)
        renderer.queueNode(self, mesh = self._button_background_copy, shader = self._button_mesh_copy,  backface_cull = True,sort = -10)
        renderer.queueNode(self, mesh = self._button_background_flat, shader = self._button_mesh_flat,  backface_cull = True,sort = -10)
        renderer.queueNode(self, mesh = self._button_background_delete, shader = self._button_mesh_delete,  backface_cull = True,sort = -10)
        renderer.queueNode(self, mesh = self._button_background_open, shader = self._button_mesh_open, backface_cull = True,sort = -10)
    ...
    def setActiveAxisforopen(self) -> None:
        # self._global_container_stack = self._application.getGlobalContainerStack()
        # if self._global_container_stack.definition.getName() in["i-fast"," X-CF Pro","X-MAX","X-Plus"]:
            # self._button_mesh_open.setUniformValue("u_color", Color(0.25,0.5,1.0,1.0))
        # else:
        self._button_mesh_open.setUniformValue("u_color", Color(0.72,0.77,0.9,1.0))
        self._scene.sceneChanged.emit(self)

    def setDisableAxisforopen(self) -> None:
        if not self._shader:
            return
        self._button_mesh_open.setUniformValue("u_color", Color(0.0,0.0,0.0,0.0))
        self._scene.sceneChanged.emit(self)

    def setActiveAxisfordelete(self) -> None:
        # if self._global_container_stack.definition.getName() in["i-fast"," X-CF Pro","X-MAX","X-Plus"]:
            # self._button_mesh_delete.setUniformValue("u_color", Color(0.25,0.5,1.0,1.0))
        # else:
            # self._button_mesh_delete.setUniformValue("u_color", Color(0.17,0.35,0.7,0.7))
        self._button_mesh_delete.setUniformValue("u_color", Color(0.72,0.77,0.9,1.0))

        self._scene.sceneChanged.emit(self)

    def setDisableAxisfordelete(self) -> None:
        if not self._shader:
            return
        self._button_mesh_delete.setUniformValue("u_color", Color(0.0,0.0,0.0,0.0))
        self._scene.sceneChanged.emit(self)

    def setActiveAxisforflat(self) -> None:
        # if self._global_container_stack.definition.getName() in["i-fast"," X-CF Pro","X-MAX","X-Plus"]:
            # self._button_mesh_flat.setUniformValue("u_color", Color(0.25,0.5,1.0,1.0))
        # else:
            # self._button_mesh_flat.setUniformValue("u_color", Color(0.17,0.35,0.7,0.7))
        self._button_mesh_flat.setUniformValue("u_color", Color(0.72,0.77,0.9,1.0))

        self._scene.sceneChanged.emit(self)

    def setDisableAxisforflat(self) -> None:
        if not self._shader:
            return
        self._button_mesh_flat.setUniformValue("u_color", Color(0.0,0.0,0.0,0.0))
        self._scene.sceneChanged.emit(self)

    def setActiveAxisforcopy(self) -> None:
        # if self._global_container_stack.definition.getName() in["i-fast"," X-CF Pro","X-MAX","X-Plus"]:
            # self._button_mesh_copy.setUniformValue("u_color", Color(0.25,0.5,1.0,1.0))
        # else:
            # self._button_mesh_copy.setUniformValue("u_color", Color(0.17,0.35,0.7,0.7))
        self._button_mesh_copy.setUniformValue("u_color", Color(0.72,0.77,0.9,1.0))

        self._scene.sceneChanged.emit(self)

    def setDisableAxisforcopy(self) -> None:
        if not self._shader:
            return
        self._button_mesh_copy.setUniformValue("u_color", Color(0.0,0.0,0.0,0.0))
        self._scene.sceneChanged.emit(self)


    def setActiveAxisforarrange(self) -> None:
        # if self._global_container_stack.definition.getName() in["i-fast"," X-CF Pro","X-MAX","X-Plus"]:
            # self._button_mesh_arrange.setUniformValue("u_color", Color(0.25,0.5,1.0,1.0))
        # else:
            # self._button_mesh_arrange.setUniformValue("u_color", Color(0.17,0.35,0.7,0.7))
        self._button_mesh_arrange.setUniformValue("u_color", Color(0.72,0.77,0.9,1.0))

        self._scene.sceneChanged.emit(self)

    def setDisableAxisforarrange(self) -> None:
        if not self._shader:
            return
        self._button_mesh_arrange.setUniformValue("u_color", Color(0.0,0.0,0.0,0.0))
        self._scene.sceneChanged.emit(self)
    ...
    def updateNodeBoundaryCheck(self):
        ...
        if build_volume_bounding_box:
            ...
            # No bounding box. This is triggered when running QIDI from command line with a model for the first time
        ...
        for node in nodes:
            ...
            if node.callDecoration("isSliceable") or node.callDecoration("isGroup"):
                if not isinstance(node, QIDISceneNode):
                    continue
    ...
    def checkBoundsAndUpdate(self, node: QIDISceneNode, bounds: Optional[AxisAlignedBox] = None) -> None:
        ...
        if not isinstance(node, QIDISceneNode) or self._global_container_stack is None:
            return
    ...
    def _buildMesh(self, min_w: float, max_w: float, min_h: float, max_h: float, min_d: float, max_d:float, z_fight_distance: float) -> MeshData:
        if self._shape != "elliptic":
            ...
            mb.addLine(Vector(max_w-(max_w-min_w)/2-6, min_h, max_d), Vector(max_w-(max_w-min_w)/2, min_h, max_d+6), color = self._volume_outline_color)
            mb.addLine(Vector(max_w-(max_w-min_w)/2+6, min_h, max_d), Vector(max_w-(max_w-min_w)/2, min_h, max_d+6), color = self._volume_outline_color)
    ...
    def _buildButtonMesh_arrange(self, origin: Vector) -> MeshData:
        ...
        if self._global_container_stack:
            ...
            open_offset = container.getMetaDataEntry("open_button_offset")
            delete_offset = container.getMetaDataEntry("delete_button_offset")
            flat_offset = container.getMetaDataEntry("flat_button_offset")
            copy_offset = container.getMetaDataEntry("copy_button_offset")
            arrange_offset = container.getMetaDataEntry("arrange_button_offset")
            
            if offset and open_offset and delete_offset:
                mb.addCube(
                    width=18, #20
                    height=0.1,
                    depth=18, #1.5
                    # center=Vector(-165-arrange_offset[0]+open_offset[0], -1-arrange_offset[1]+open_offset[1], 115+arrange_offset[2]-open_offset[2]),
                    center = self.offset_Vector+Vector(arrange_offset[0], arrange_offset[1]+0.1, arrange_offset[2]),
                    color=Color(0.0,0.0,0.5,0.3)
                )

        return mb.build()

    def _buildButtonMesh_copy(self, origin: Vector) -> MeshData:
        ...
        if self._global_container_stack:
            ...
            if offset and open_offset and delete_offset:
                mb.addCube(
                    ...
                    height=0.1,
                    depth=18, #1.5
                    # center=Vector(-165-copy_offset[0]+open_offset[0], -1-copy_offset[1]+open_offset[1], 115+copy_offset[2]-open_offset[2]),
                    center = self.offset_Vector+Vector(copy_offset[0], copy_offset[1]+0.1, copy_offset[2]),
    ...
    def _buildButtonMesh_flat(self, origin: Vector) -> MeshData:
        ...
        if self._global_container_stack:
            ...
            if offset and open_offset and delete_offset:
                mb.addCube(
                    ...
                    height=0.1,
                    depth=18, #1.5
                    # center=Vector(-165-float_offset[0]+open_offset[0], -1-float_offset[1]+open_offset[1], 115+float_offset[2]-open_offset[2]),
                    center = self.offset_Vector+Vector(flat_offset[0], flat_offset[1]+0.1, flat_offset[2]),
    ...
    def _buildButtonMesh_delete(self, origin: Vector) -> MeshData:
        ...
        if self._global_container_stack:
            ...
            if offset and open_offset and delete_offset:
                mb.addCube(
                    ...
                    height=0.1,
                    depth=18, #1.5
                    # center=Vector(-165-delete_offset[0]+open_offset[0], -1-delete_offset[1]+open_offset[1], 115+delete_offset[2]-open_offset[2]),
                    center = self.offset_Vector+Vector(delete_offset[0], delete_offset[1]+0.1, delete_offset[2]),
    ...
    def _buildButtonMesh_open(self, origin: Vector) -> MeshData:
        ...
        if self._global_container_stack:
            ...
            if offset and open_offset and delete_offset:
                mb.addCube(
                    ...
                    height=0.1,
                    depth=18, #1.5
                    # center=Vector(-165-open_offset[0]+open_offset[0], -1-open_offset[1]+open_offset[1], 115+open_offset[2]-open_offset[2]),
                    center = self.offset_Vector+Vector(open_offset[0], open_offset[1]+0.1, open_offset[2]),
    ...
    def _buildOriginMesh(self, origin: Vector) -> MeshData:
        ...
        mb.addCube(
            width=self._origin_line_length, #20
            height=self._origin_line_width/2,
            depth=self._origin_line_width/2, #1.5
        ...
        mb.addCube(
            width=self._origin_line_width/2,
            height=self._origin_line_length,
            depth=self._origin_line_width/2,
        ...
        mb.createCube(
            verts=[
                [min_w + 22, min_h - 0.5, max_d + 2.5],
                [min_w + 22, min_h + 0.5, max_d + 2.5],
                [min_w + 28, min_h + 0.5, max_d - 1.5],
                [min_w + 28, min_h - 0.5, max_d - 1.5],
                [min_w + 22, min_h - 0.5, max_d + 1.5],
                [min_w + 22, min_h + 0.5, max_d + 1.5],
                [min_w + 28, min_h + 0.5, max_d - 2.5],
                [min_w + 28, min_h - 0.5, max_d - 2.5]
            ],
            color=self._x_axis_color
        )
        ...
        mb.createCube(
            verts=[
                [min_w - 0.5, min_h - 0.5, max_d - 22],
                [min_w - 0.5, min_h + 0.5, max_d - 22],
                [min_w + 0.5, min_h + 0.5, max_d - 22],
                [min_w + 0.5, min_h - 0.5, max_d - 22],
                [min_w - 0.5, min_h - 0.5, max_d - 25],
                [min_w - 0.5, min_h + 0.5, max_d - 25],
                [min_w + 0.5, min_h + 0.5, max_d - 25],
                [min_w + 0.5, min_h - 0.5, max_d - 25]
            ],
        ...
        mb.createCube(
            verts=[
                ...
                [min_w + 0.5, min_h + 0.5, max_d - 25],
                [min_w + 0.5, min_h - 0.5, max_d - 25],
                [min_w - 2.5, min_h - 0.5, max_d - 28],
                [min_w - 2.5, min_h + 0.5, max_d - 28],
                [min_w - 1.5, min_h + 0.5, max_d - 28],
                [min_w - 1.5, min_h - 0.5, max_d - 28]
            ],
            color=self._z_axis_color
        )
        ...
        mb.createCube(
            verts=[
                [min_w - 2.5, min_h + 27, max_d + 0.5],
                [min_w - 2.5, min_h + 28, max_d + 0.5],
                [min_w + 2.5, min_h + 28, max_d + 0.5],
                [min_w + 2.5, min_h + 27, max_d + 0.5],
                [min_w - 2.5, min_h + 27, max_d - 0.5],
                [min_w - 2.5, min_h + 28, max_d - 0.5],
        ...
        mb.createCube(
            verts=[
                [min_w - 2.5, min_h + 22, max_d + 0.5],
                [min_w - 2.5, min_h + 23, max_d + 0.5],
                [min_w + 2.5, min_h + 28, max_d + 0.5],
                [min_w + 2.5, min_h + 27, max_d + 0.5],
                [min_w - 2.5, min_h + 22, max_d - 0.5],
                [min_w - 2.5, min_h + 23, max_d - 0.5],
        ...
        # ToolHandle.setSolidMesh(mb.build())
        
        # mb.setFileName(r"G:\QIDIWrite\QIDI\build\package\resources\meshes\tubiao.STL")

        ###########
        # return mb.build()
        return mb.build()

    def _buildButtonMesh(self, origin: Vector) -> MeshData:
        ...
        if self._global_container_stack:
            ...
            copy_offset = container.getMetaDataEntry("copy_button_offset")
            arrange_offset = container.getMetaDataEntry("arrange_button_offset")
            if self._global_container_stack.definition.getName() == "X-MAX 3":
                self.offset_Vector = Vector(0, 0, 0)
            elif self._global_container_stack.definition.getName() == "X-Plus 3":
                self.offset_Vector = Vector(0, 0, 0)
            elif self._global_container_stack.definition.getName() == "X-mate 3":
                self.offset_Vector = Vector(0, 0, 0)
            else:
                self.offset_Vector = Vector(0, 0, 0)

            # self.setPosition(Vector(0,0,0))
            # Logger.log("e",self.getPosition())
            # Logger.log("e",self.getPosition())
            if offset and open_offset and delete_offset:
                mb = MeshBuilder()
                mb.addCube(
                    ...
                    depth=22,
                    center=self.offset_Vector+Vector(open_offset[0], open_offset[1], open_offset[2]),
                    color=self._open_file_color
                )
                mb.addCube(
                    width=22,
                    height=self._origin_line_width,
                    depth=22,
                    center=self.offset_Vector+Vector(delete_offset[0], delete_offset[1], delete_offset[2]),
                    color=self._delete_file_color
                )
                mb.addCube(
                    width=22,
                    height=self._origin_line_width,
                    depth=22,
                    center=self.offset_Vector+Vector(flat_offset[0], flat_offset[1], flat_offset[2]),
                    color=self._flat_file_color
                )
                mb.addCube(
                    width=22,
                    height=self._origin_line_width,
                    depth=22,
                    center=self.offset_Vector+Vector(copy_offset[0], copy_offset[1], copy_offset[2]),
                    color=self._copy_file_color
                )
                mb.addCube(
                    width=22,
                    height=self._origin_line_width,
                    depth=22,
                    center=self.offset_Vector+Vector(arrange_offset[0], arrange_offset[1], arrange_offset[2]),
                    color=self._place_file_color
                )
                Application.getInstance().setButtonMesh(mb.build())
                return
        return mb.build()
    ...
    def _updateColors(self):
        ...
        self._volume_outline_color = Color(*theme.getColor("blue_2").getRgb())
        self._x_axis_color = Color(*theme.getColor("x_axis").getRgb())
        self._y_axis_color = Color(*theme.getColor("y_axis").getRgb())
        self._z_axis_color = Color(*theme.getColor("z_axis").getRgb())
        self._open_file_color =Color(*theme.getColor("open_file").getRgb()) 
        self._delete_file_color =Color(*theme.getColor("delete_file").getRgb()) 
        self._flat_file_color =Color(*theme.getColor("flat_file").getRgb()) 
        self._copy_file_color =Color(*theme.getColor("copy_file").getRgb()) 
        self._place_file_color =Color(*theme.getColor("place_file").getRgb()) 
    ...
    def _buildDisallowedAreaMesh(self, min_w: float, max_w: float, min_h: float, max_h: float, min_d: float, max_d: float, disallowed_area_height: float) -> Optional[MeshData]:
        ...
        for polygon in self._disallowed_areas:
            ...
            # This is a very nasty hack. This pretty much only works for QD machines.
    ...
    def rebuild(self) -> None:
        ...
        if not self._volume_outline_color:
            # self._platformbutton = PlatformForButton(self,origin)
            self._updateColors()
        ...
        # Logger.log("e",self._global_container_stack.definition.getName())
        ...
        # self.setPlatformbutton()
        
        if self._global_container_stack.definition.getName() == "i-fast":
            origin = origin+Vector(-15, 0, 0)
        self._buildButtonMesh(origin)
        self._button_background_arrange = self._buildButtonMesh_arrange(origin)
        self._button_background_copy = self._buildButtonMesh_copy(origin)
        self._button_background_flat = self._buildButtonMesh_flat(origin)
        self._button_background_delete = self._buildButtonMesh_delete(origin)
        self._button_background_open = self._buildButtonMesh_open(origin)
        ...
        # As this works better for QD machines, we only add the disallowed_area_size for the z direction.
    ...
    # def setPlatformbutton(self):
        # self._platformbutton = None
        # self._platformbutton = PlatformForButton(self,self._origin)
    ...
    def _updateDisallowedAreasAndRebuild(self):
        """Calls :py:meth:`qidi.BuildVolume._updateDisallowedAreas` and makes sure the changes appear in the scene.

        This is required for a signal to trigger the update in one go. The
        :py:meth:`qidi.BuildVolume._updateDisallowedAreas` method itself shouldn't call
        :py:meth:`qidi.BuildVolume.rebuild`, since there may be other changes before it needs to be rebuilt,
    ...
    def _computeDisallowedAreasPrinted(self, used_extruders):
        ...
        for extruder in used_extruders:
            if adhesion_extruder == -1:
                adhesion_extruder = used_extruders[0]  # The prime tower brim is always printed with the first extruder
            elif int(extruder.getProperty("extruder_nr", "value")) == int(self._global_container_stack.getProperty("adhesion_extruder_nr", "value")):
                adhesion_extruder = extruder
            result[extruder.getId()] = []
    ...
    def _computeDisallowedAreasStatic(self, border_size:float, used_extruders: List["ExtruderStack"]) -> Dict[str, List[Polygon]]:
        ...
        for extruder in used_extruders:
            ...
            if offset_y is None:
                offset_y = 0
            offset_y = -offset_y  # Y direction of g-code is the inverse of Y direction of QIDI's scene space.
            result[extruder_id] = []
