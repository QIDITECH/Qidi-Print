...
if MYPY:
    ...
    try:
        from qidi.QIDIVersion import QIDIDebugMode
    except ImportError:
        QIDIDebugMode = False  # [CodeStyle: Reflecting imported value]
...
class CrashHandler:
    def __init__(self, exception_type, value, tb, has_started = True):
        ...
        self.qidi_version = None
        self.qidi_locale = None
        ...
        for line in traceback.format_exception(exception_type, value, tb):
            for part in line.rstrip("\n").split("\n"):
                Logger.log("c", part)
        self.data = {}
        # If QIDI has fully started, we only show fatal errors.
        # If QIDI has not fully started yet, we always show the early crash dialog. Otherwise, QIDI will just crash
    ...
    def _createEarlyCrashDialog(self):
        ...
        dialog.setWindowTitle(catalog.i18nc("@title:window", "QIDI can't start"))
        dialog.finished.connect(self._closeEarlyCrashDialog)

        layout = QVBoxLayout(dialog)

        label = QLabel()
        label.setText(catalog.i18nc("@label crash message", """<p><b>Oops, QIDI TECH has encountered something that doesn't seem right.</p></b>
    ...
    def _messageWidget(self):
        label = QLabel()
        label.setText(catalog.i18nc("@label crash message", """<p><b>A fatal error has occurred in QIDI. Please send us this Crash Report to fix the problem</p></b>
    ...
    def _informationWidget(self):
        ...
        try:
            from QD.Application import Application
            self.qidi_version = Application.getInstance().getVersion()
            self.qidi_locale = Application.getInstance().getPreferences().getValue("general/language")
        except:
            self.qidi_version = catalog.i18nc("@label unknown version of QIDI", "Unknown")
            self.qidi_locale = "??_??"

        self.data["qidi_version"] = self.qidi_version
        ...
        self.data["locale_qidi"] = self.qidi_locale

        try:
            from qidi.QIDIApplication import QIDIApplication
            plugins = QIDIApplication.getInstance().getPluginRegistry()
            self.data["plugins"] = {
        ...
        crash_info = "<b>" + catalog.i18nc("@label QIDI version number", "QIDI version") + ":</b> " + str(self.qidi_version) + "<br/>"
        crash_info += "<b>" + catalog.i18nc("@label", "QIDI language") + ":</b> " + str(self.qidi_locale) + "<br/>"
        ...
        if with_sentry_sdk:
            with configure_scope() as scope:
                ...
                scope.set_tag("locale_qidi", self.qidi_locale)
                scope.set_tag("is_enterprise", ApplicationMetadata.IsEnterpriseVersion)

                scope.set_context("plugins", self.data["plugins"])

                user_id = uuid.getnode()  # On all of QIDI's supported platforms, this returns the MAC address which is pseudonymical information (!= anonymous).
    ...
    def _getOpenGLInfo(self):
        ...
        try:
            from qidi.QIDIApplication import QIDIApplication
            application = cast(QIDIApplication, Application.getInstance())
    ...
    def _exceptionInfoWidget(self):
        ...
        split_path = []
        folder_name = ""
        # Split until reach folder "qidi"
        while folder_name != "qidi":
            directory, folder_name = os.path.split(directory)
            if not folder_name:
                break
            split_path.append(folder_name)

        # Look for plugins. If it's not a plugin, the current qidi version is set
        isPlugin = False
        module_version = self.qidi_version
        module_name = "QIDI"
    ...
    def _logInfoWidget(self):
        ...
        text_area = QTextEdit()
        tmp_file_fd, tmp_file_path = tempfile.mkstemp(prefix = "qidi-crash", text = True)
