...
from QD.FileHandler.FileHandler import FileHandler #For typing.
...
class NetworkedPrinterOutputDevice(PrinterOutputDevice):
    ...
    def __init__(self, device_id, address: str, properties: Dict[bytes, bytes], connection_type: ConnectionType = ConnectionType.NetworkConnection, parent: QObject = None) -> None:
        ...
        self._user_agent = "%s/%s " % (QIDIApplication.getInstance().getApplicationName(),
                                       QIDIApplication.getInstance().getVersion())
    ...
    def _getUserName(self) -> str:
        ...
        account = QIDIApplication.getInstance().getQIDIAPI().account  # type: Account
    ...
    def _checkCorrectGroupName(self, device_id: str, group_name: str) -> None:
        ...
        active_machine_network_name = QIDIApplication.getInstance().getMachineManager().activeMachineNetworkKey()
        if global_container_stack and device_id == active_machine_network_name:
            ...
            if QIDIApplication.getInstance().getMachineManager().activeMachineNetworkGroupName != group_name:
                metadata_filter = {"um_network_key": active_machine_network_name}
                containers = QIDIContainerRegistry.getInstance().findContainerStacks(type="machine",
                                                                                     **metadata_filter)
