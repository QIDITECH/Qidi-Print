...
class PrintInformation(QObject):
    ...
    def __init__(self, application: "QIDIApplication", parent = None) -> None:
        ...
        self.initializeQIDIMessagePrintTimeProperties()
        ...
        self._gcode_time=Duration(parent = self)
    ...
    def initializeQIDIMessagePrintTimeProperties(self) -> None:
    ...
    def _calculateInformation(self, build_plate_number: int) -> None:
        ...
        try:
            material_preference_values = json.loads(self._application.getInstance().getPreferences().getValue("qidi/material_settings"))
    ...
    def _onPreferencesChanged(self, preference: str) -> None:
        if preference != "qidi/material_settings":
            return
    ...
    def _updateJobName(self) -> None:
        ...
        if not self._is_user_specified_job_name:
            if self._application.getInstance().getPreferences().getValue("qidi/jobname_prefix") and not self._pre_sliced:
    ...
    def setBaseName(self, base_name: str, is_project_file: bool = False) -> None:
        ...
        if is_gcode or is_project_file or (is_empty or (self._base_name == "" and self._base_name != check_name)):
            ...
            # Strip the old "qidiproject" extension from the name
            OLD_QIDI_PROJECT_EXT = ".qidiproject"
            if self._base_name.lower().endswith(OLD_QIDI_PROJECT_EXT):
                self._base_name = self._base_name[:len(self._base_name) - len(OLD_QIDI_PROJECT_EXT)]

            # QIDI-5896 Try to strip extra extensions with an infinite amount of ".qidiproject.3mf".
            OLD_QIDI_PROJECT_3MF_EXT = ".qidiproject.3mf"
            while self._base_name.lower().endswith(OLD_QIDI_PROJECT_3MF_EXT):
                self._base_name = self._base_name[:len(self._base_name) - len(OLD_QIDI_PROJECT_3MF_EXT)]
