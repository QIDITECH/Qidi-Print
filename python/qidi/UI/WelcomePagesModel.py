...
class WelcomePagesModel(ListModel):
    ...
    def _getBuiltinWelcomePagePath(self, page_filename: str) -> "QUrl":
        from qidi.QIDIApplication import QIDIApplication
        return QUrl.fromLocalFile(Resources.getPath(QIDIApplication.ResourceTypes.QmlFiles,
                                                    os.path.join("WelcomePages", page_filename)))

    def _getBuiltinUseWizardPath(self, page_filename: str) -> "QUrl":
        from qidi.QIDIApplication import QIDIApplication
        return QUrl.fromLocalFile(Resources.getPath(QIDIApplication.ResourceTypes.QmlFiles,
                                                    os.path.join("UseWizard", page_filename)))
    ...
    def shouldShowCloudPage(self) -> bool:
        ...
        """
        # Import QIDIApplication locally or else it fails
        from qidi.QIDIApplication import QIDIApplication
        api = QIDIApplication.getInstance().getQIDIAPI()
