...
#
class ReportABugModel(WelcomePagesModel):
...
#                            })
        self._pages.append({"id": "changelog",
                            "page_url": self._getBuiltinWelcomePagePath("ReportEmailContent.qml"),
                            "next_page_button_text": self._catalog.i18nc("@action:button", "Close"),
                            })
        self.setItems(self._pages)

        images, max_image = ReportABugModel._collectOrdinalFiles(Resources.Images, ReportABugModel.image_formats)
        texts, max_text = ReportABugModel._collectOrdinalFiles(Resources.Texts, ReportABugModel.text_formats)
        ...
        for n in range(0, highest + 1):
            self._subpages.append({
                ReportABugModel.image_key: None if n not in images else images[n],
                ReportABugModel.text_key: None if n not in texts else self._loadText(texts[n])
            })
        if len(self._subpages) == 0:
            self._subpages.append({ReportABugModel.text_key: "~ There Is Nothing New Under The Sun ~"})
    ...
    def getSubpageImageSource(self, page: int) -> str:
        result = self._getSubpageItem(page, ReportABugModel.image_key)
        return "file:///" + (result if result else Resources.getPath(Resources.Images, "qidi-icon.png"))

    @pyqtSlot(int, result = str)
    def getSubpageText(self, page: int) -> str:
        result = self._getSubpageItem(page, ReportABugModel.text_key)
        return result if result else "* * *"

__all__ = ["ReportABugModel"]
