...
from QD.Math.Polygon import Polygon  # For typing.
from QD.Scene.SceneNode import SceneNode
from QD.Scene.SceneNodeDecorator import SceneNodeDecorator  # To cast the deepcopy of every decorator back to SceneNodeDecorator.

import qidi.QIDIApplication  # To get the build plate.
from qidi.Settings.ExtruderStack import ExtruderStack  # For typing.
from qidi.Settings.SettingOverrideDecorator import SettingOverrideDecorator  # For per-object settings.


class QIDISceneNode(SceneNode):
    """Scene nodes that are models are only seen when selecting the corresponding build plate

    Note that many other nodes can just be QD SceneNode objects.
    """
    ...
    def isVisible(self) -> bool:
        return super().isVisible() and self.callDecoration("getBuildPlateNumber") == qidi.QIDIApplication.QIDIApplication.getInstance().getMultiBuildPlateModel().activeBuildPlate

    def isSelectable(self) -> bool:
        return super().isSelectable() and self.callDecoration("getBuildPlateNumber") == qidi.QIDIApplication.QIDIApplication.getInstance().getMultiBuildPlateModel().activeBuildPlate
    ...
    def getDiffuseColor(self) -> List[float]:
        ...
        position = printing_extruder.getMetaDataEntry("position", default = "0")
        ...
        if global_container_stack:
            ...
            if Application.getInstance().getPreferences().getValue("view/use_extruder_color") and machine_extruder_count > 1:
                if Application.getInstance().getPreferences().getValue("color/extruder" + str(position)):
                    material_color = str(Application.getInstance().getPreferences().getValue("color/extruder" + str(position)))
                else:
                    material_color
    ...
    def __deepcopy__(self, memo: Dict[int, object]) -> "QIDISceneNode":
        """Taken from SceneNode, but replaced SceneNode with QIDISceneNode"""

        copy = QIDISceneNode(no_setting_override = True)  # Setting override will be added later
