...
from qidi.Scene import ConvexHullNode
...
class ConvexHullDecorator(SceneNodeDecorator):
    ...
    def __init__(self) -> None:
        ...
        if QIDIApplication.getInstance() is not None:
            self._timer_scheduled_to_be_created = True
            QIDIApplication.getInstance().callLater(self.createRecomputeConvexHullTimer)

        self._raft_thickness = 0.0
        self._build_volume = QIDIApplication.getInstance().getBuildVolume()
        self._build_volume.raftThicknessChanged.connect(self._onChanged)

        QIDIApplication.getInstance().globalContainerStackChanged.connect(self._onGlobalStackChanged)
        controller = QIDIApplication.getInstance().getController()
        controller.toolOperationStarted.connect(self._onChanged)
        controller.toolOperationStopped.connect(self._onChanged)
        #QIDIApplication.getInstance().sceneBoundingBoxChanged.connect(self._onChanged)
    ...
    def recomputeConvexHullDelayed(self) -> None:
        ...
        if self._recompute_convex_hull_timer is not None:
            ...
            QIDIApplication.getInstance().callLater(self.recomputeConvexHullDelayed)
