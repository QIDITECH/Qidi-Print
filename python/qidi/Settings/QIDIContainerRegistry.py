...
from QD.PluginRegistry import PluginRegistry  # For getting the possible profile writers to write with.
...
from qidi.Settings.qidi_empty_instance_containers import empty_quality_container
from qidi.Machines.ContainerTree import ContainerTree
from qidi.ReaderWriters.ProfileReader import NoProfileException, ProfileReader

from QD.i18n import i18nCatalog
catalog = i18nCatalog("qidi")


class QIDIContainerRegistry(ContainerRegistry):
    ...
    def addContainer(self, container: ContainerInterface) -> bool:
        ...
        if isinstance(container, InstanceContainer) and type(container) != type(self.getEmptyInstanceContainer()):
            ...
            required_setting_version = qidi.QIDIApplication.QIDIApplication.SettingVersion
    ...
    def importProfile(self, file_name: str) -> Dict[str, str]:
        ...
        for plugin_id, meta_data in self._getIOPlugins("profile_reader"):
            ...
            if profile_or_list:
                ...
                # i.e.: We expect gcode for a QD2 Extended to be defined as normal QD2 gcode...
                ...
                if len(profile_or_list) == 1:
                    ...
                    for idx, extruder in enumerate(global_stack.extruderList):
                        ...
                        profile.setMetaDataEntry("setting_version", qidi.QIDIApplication.QIDIApplication.SettingVersion)
    ...
    def _isMetadataValid(self, metadata: Optional[Dict[str, Any]]) -> bool:
        ...
        This overrides the one from QD.Settings.ContainerRegistry because we
        ...
        if "setting_version" not in metadata:
            return False
        try:
            if int(metadata["setting_version"]) != qidi.QIDIApplication.QIDIApplication.SettingVersion:
                return False
    ...
    def addExtruderStackForSingleExtrusionMachine(self, machine, extruder_id, new_global_quality_changes = None, create_new_ids = True):
        ...
        if machine_quality_changes.getId() not in ("empty", "empty_quality_changes"):
            ...
            if extruder_quality_changes_container:
                ...
                extruder_quality_changes_container = self._findQualityChangesContainerInQIDIFolder(machine_quality_changes.getName())
                if extruder_quality_changes_container:
                    quality_changes_id = extruder_quality_changes_container.getId()
        ...
        for qc in qcs:
            ...
            # Try to find from the quality changes qidi directory too
            quality_changes_container = self._findQualityChangesContainerInQIDIFolder(machine_quality_changes.getName())
    ...
    def _findQualityChangesContainerInQIDIFolder(self, name: str) -> Optional[InstanceContainer]:
        quality_changes_dir = Resources.getPath(qidi.QIDIApplication.QIDIApplication.ResourceTypes.QualityChangesInstanceContainer)
    ...
    @classmethod
    @override(ContainerRegistry)
    def getInstance(cls, *args, **kwargs) -> "QIDIContainerRegistry":
        return cast(QIDIContainerRegistry, super().getInstance(*args, **kwargs))
