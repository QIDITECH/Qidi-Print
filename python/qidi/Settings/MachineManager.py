...
from QD.ConfigurationErrorMessage import ConfigurationErrorMessage
...
from QD.Signal import postponeSignals, CompressTechnique

import qidi.QIDIApplication  # Imported like this to prevent circular references.
...
from qidi.PrinterOutput.PrinterOutputDevice import PrinterOutputDevice, ConnectionType
from qidi.PrinterOutput.Models.PrinterConfigurationModel import PrinterConfigurationModel
from qidi.PrinterOutput.Models.ExtruderConfigurationModel import ExtruderConfigurationModel
from qidi.PrinterOutput.Models.MaterialOutputModel import MaterialOutputModel
...
from qidi.Settings.qidi_empty_instance_containers import (empty_definition_changes_container, empty_variant_container,
                                                          empty_material_container, empty_quality_container,
                                                          empty_quality_changes_container, empty_intent_container)
from qidi.QIDICloud.QIDICloudConstants import META_QD_LINKED_TO_ACCOUNT

from .QIDIStackBuilder import QIDIStackBuilder
...
if TYPE_CHECKING:
    ...
    from qidi.Machines.QualityChangesGroup import QualityChangesGroup
    from qidi.Machines.QualityGroup import QualityGroup
    from qidi.Machines.VariantNode import VariantNode
...
class MachineManager(QObject):
    def __init__(self, application: "QIDIApplication", parent: Optional["QObject"] = None) -> None:
        ...
        self._current_root_material_is_read = {}
        ...
        self._application.getPreferences().addPreference("qidi/active_machine", "")
        ...
        containers = QIDIContainerRegistry.getInstance().findInstanceContainers(id = self.activeMaterialId)  # type: List[InstanceContainer]
        if containers:
    ...
    def setInitialActiveMachine(self) -> None:
        active_machine_id = self._application.getPreferences().getValue("qidi/active_machine")
        if active_machine_id != "" and QIDIContainerRegistry.getInstance().findContainerStacksMetadata(id = active_machine_id):
    ...
    def _onCurrentConfigurationChanged(self) -> None:
        ...
        # Create the configuration model with the current data in QIDI
    ...
    def getAllSettingKeys(self) -> Set[str]:
        general_definition_containers = QIDIContainerRegistry.getInstance().findDefinitionContainers(id="fdmprinter")
        if not general_definition_containers:
            return set()
        return general_definition_containers[0].getAllKeys()

    def _onGlobalContainerChanged(self) -> None:
        """Triggered when the global container stack is changed in QIDIApplication."""
        ...
        if self._global_container_stack:
            self._application.getPreferences().setValue("qidi/active_machine", self._global_container_stack.getId())
            self._application.getPreferences().setValue("qidi/active_mode",1)
            ...
            # Set the global material to empty as we now use the extruder stack at all times - QIDI-4482
    ...
    def setActiveMachine(self, stack_id: Optional[str]) -> None:
        ...
        container_registry = QIDIContainerRegistry.getInstance()
    ...
    def getMachine(definition_id: str, metadata_filter: Optional[Dict[str, str]] = None) -> Optional["GlobalStack"]:
        ...
        if metadata_filter is None:
            metadata_filter = {}
        machines = QIDIContainerRegistry.getInstance().findContainerStacks(type = "machine", **metadata_filter)
    ...
    def addMachine(self, definition_id: str, name: Optional[str] = None) -> bool:
        ...
        if name is None:
            definitions = QIDIContainerRegistry.getInstance().findDefinitionContainers(id = definition_id)
            if definitions:
                name = definitions[0].getName()
            else:
                name = definition_id

        metadatas = QIDIContainerRegistry.getInstance().findContainerStacksMetadata(name = name)
        if metadatas:
            metadata = metadatas[0]
            self.setActiveMachine(metadata["id"])

            return True

        new_stack = QIDIStackBuilder.createMachine(cast(str, name), definition_id)
        if new_stack:
    ...
    def activeMachineIsLinkedToCurrentAccount(self) -> bool:
        return parseBool(self.activeMachine.getMetaDataEntry(META_QD_LINKED_TO_ACCOUNT, "True"))
    ...
    def removeMachine(self, machine_id: str) -> None:
        ...
        if activate_new_machine:
            machine_stacks = QIDIContainerRegistry.getInstance().findContainerStacksMetadata(type = "machine")
        ...
        metadatas = QIDIContainerRegistry.getInstance().findContainerStacksMetadata(id = machine_id)
        if not metadatas:
            return  # machine_id doesn't exist. Nothing to remove.
        metadata = metadatas[0]
        ExtruderManager.getInstance().removeMachineExtruders(machine_id)
        containers = QIDIContainerRegistry.getInstance().findInstanceContainersMetadata(type = "user", machine = machine_id)
        for container in containers:
            QIDIContainerRegistry.getInstance().removeContainer(container["id"])
        machine_stacks = QIDIContainerRegistry.getInstance().findContainerStacks(type = "machine", name = machine_id)
        if machine_stacks:
            QIDIContainerRegistry.getInstance().removeContainer(machine_stacks[0].definitionChanges.getId())
        QIDIContainerRegistry.getInstance().removeContainer(machine_id)

        # If the printer that is being removed is a network printer, the hidden printers have to be also removed
        group_id = metadata.get("group_id", None)
        if group_id:
            metadata_filter = {"group_id": group_id, "hidden": True}
            hidden_containers = QIDIContainerRegistry.getInstance().findContainerStacks(type = "machine", **metadata_filter)
    ...
    def getDefinitionByMachineId(self, machine_id: str) -> Optional[str]:
        ...
        """
        containers = QIDIContainerRegistry.getInstance().findContainerStacks(id = machine_id)
        if containers:
    ...
    def correctPrintSequence(self) -> None:
        ...
        QIDI UI is properly updated to reset all the UI elements changes that occur due to the one-at-a-time mode (such
    ...
    def setActiveMachineExtruderCount(self, extruder_count: int) -> None:
        ...
        # After QIDI-4482 this should not be the case anymore, but we still want to support older project files.
        ...
        if self._global_container_stack:
            for extruder in self._global_container_stack.extruderList:
                ...
                if position not in self._current_root_material_id or material_id != self._current_root_material_id[position]:
                    ...
                    self._current_root_material_is_read[position] = extruder.material.getMetaDataEntry("brand", "unknown").lower()
    ...
    @pyqtProperty("QVariant", notify = rootMaterialChanged)
    def currentRootMaterialIsRead(self) -> Dict[str, str]:
        return self._current_root_material_is_read
    ...
    def _setMaterialtest(self, position: str, material_node: Optional["MaterialNode"] = None) -> None:
        if self._global_container_stack is None:
            return
        if material_node and material_node.container:
            material_container = material_node.container
            ...
            testmaterial=self._global_container_stack.extruderList[int(position)].material
            self._global_container_stack.extruderList[int(position)].material = empty_material_container
            self._global_container_stack.extruderList[int(position)].material = testmaterial
            #root_material_id = None
        # The _current_root_material_id is used in the MaterialMenu to see which material is selected
    ...
    def switchPrinterType(self, machine_name: str) -> None:
        ...
        definitions = QIDIContainerRegistry.getInstance().findDefinitionContainers(name=machine_name)
        if not definitions:
        ...
        if not new_machine:
            new_machine = QIDIStackBuilder.createMachine(machine_definition_id + "_sync", machine_definition_id)
            if not new_machine:
    ...
    def applyRemoteConfiguration(self, configuration: PrinterConfigurationModel) -> None:
        ...
        with postponeSignals(*self._getContainerChangedSignals(), compress = CompressTechnique.CompressPerParameterValue):
            ...
            for extruder_configuration in configuration.extruderConfigurations:
                ...
                if int(position) >= len(self._global_container_stack.extruderList):
                    ...
                    continue  # Remote printer gave more extruders than what QIDI had locally, e.g. because the user switched to a single-extruder printer while the sync was being processed.
        ...
        if self.hasUserSettings and self._application.getPreferences().getValue("qidi/active_mode") == 1:
    ...
    @pyqtSlot(str, "QVariant")
    def setMaterialtest(self, position: str, container_node, global_stack: Optional["GlobalStack"] = None) -> None:
        """Global_stack: if you want to provide your own global_stack instead of the current active one
        ...
        self.blurSettings.emit()
        with postponeSignals(*self._getContainerChangedSignals(), compress = CompressTechnique.CompressPerParameterValue):
            self._setMaterialtest(position, container_node)
            self._updateQualityWithMaterial()
    ...
    def setQualityGroup(self, quality_group: "QualityGroup", no_dialog: bool = False, global_stack: Optional["GlobalStack"] = None) -> None:
        ...
        # See if we need to show the Discard or Keep changes screen
        if not no_dialog and self.hasUserSettings and self._application.getPreferences().getValue("qidi/active_mode") == 1:
            self._application.discardOrKeepProfileChanges()
