...
from QD.Decorators import deprecated, override
...
class GlobalStack(QIDIContainerStack):
    ...
    def __init__(self, container_id: str) -> None:
        ...
        # TL;DR: If QIDI is looking for printers that belong to the same group, it should use "group_id".
    ...
    def getProperty(self, key: str, property_name: str, context: Optional[PropertyEvaluationContext] = None) -> Any:
        ...
        if limit_to_extruder is not None:
            if limit_to_extruder == -1:
                limit_to_extruder = int(qidi.QIDIApplication.QIDIApplication.getInstance().getMachineManager().defaultExtruderPosition)
    ...
    @override(ContainerStack)
    def setNextStack(self, stack: QIDIContainerStack, connect_signals: bool = True) -> None:
        """Overridden from ContainerStack
    ...
    def getDefaultFirmwareName(self) -> str:
        ...
        try:
            return Resources.getPath(qidi.QIDIApplication.QIDIApplication.ResourceTypes.Firmware, hex_file.format(baudrate=baudrate))
...
global_stack_mime = MimeType(
    name = "application/x-qidi-globalstack",
    comment = "QIDI Global Stack",
    suffixes = ["global.cfg"]
)
