...
from QD.SaveFile import SaveFile
...
class ContainerManager(QObject):
    """Manager class that contains common actions to deal with containers in QIDI.
    ...
    def getContainerMetaDataEntry(self, container_id: str, entry_names: str) -> str:
        metadatas = qidi.QIDIApplication.QIDIApplication.getInstance().getContainerRegistry().findContainersMetadata(id = container_id)
        if not metadatas:
    ...
    def setContainerMetaDataEntry(self, container_node: "ContainerNode", entry_name: str, entry_value: str) -> bool:
        ...
        qidi.QIDIApplication.QIDIApplication.getInstance().getMachineManager().updateUponMaterialMetadataChange()
        return True

    @pyqtSlot(str, result = str)
    def makeUniqueName(self, original_name: str) -> str:
        return qidi.QIDIApplication.QIDIApplication.getInstance().getContainerRegistry().uniqueName(original_name)
    ...
    def exportContainer(self, container_id: str, file_type: str, file_url_or_string: Union[QUrl, str]) -> Dict[str, str]:
        ...
        containers = qidi.QIDIApplication.QIDIApplication.getInstance().getContainerRegistry().findContainers(id = container_id)
        if not containers:
    ...
    def updateQualityChanges(self) -> bool:
        ...
        qidi.QIDIApplication.QIDIApplication.getInstance().getMachineManager().activeQualityChangesGroupChanged.emit()
    ...
    def clearUserContainers(self) -> None:
        ...
        machine_manager = qidi.QIDIApplication.QIDIApplication.getInstance().getMachineManager()
    ...
    def unlinkMaterial(self, material_node: "MaterialNode") -> None:
        ...
        if material_node.container is None:  # Failed to lazy-load this container.
            return
        root_material_query = qidi.QIDIApplication.QIDIApplication.getInstance().getContainerRegistry().findInstanceContainers(id = material_node.getMetaDataEntry("base_file", ""))
        if not root_material_query:
    ...
    def _updateContainerNameFilters(self) -> None:
        ...
        plugin_registry = qidi.QIDIApplication.QIDIApplication.getInstance().getPluginRegistry()
        container_registry = qidi.QIDIApplication.QIDIApplication.getInstance().getContainerRegistry()
    ...
    def importProfile(self, file_url: QUrl) -> Dict[str, str]:
        """Import single profile, file_url does not have to end with qidiprofile"""
        ...
        if not path:
            return {"status": "error", "message": catalog.i18nc("@info:status", "Invalid file URL:") + " " + str(file_url)}
        return qidi.QIDIApplication.QIDIApplication.getInstance().getContainerRegistry().importProfile(path)
    ...
    def exportQualityChangesGroup(self, quality_changes_group: "QualityChangesGroup", file_url: QUrl, file_type: str) -> None:
        ...
        qidi.QIDIApplication.QIDIApplication.getInstance().getContainerRegistry().exportQualityProfile(container_list, path, file_type)
