...
if TYPE_CHECKING:
    ...
    from qidi.BuildVolume import BuildVolume
...
def findNodePlacement(nodes_to_arrange: List["SceneNode"], build_volume: "BuildVolume", fixed_nodes: Optional[List["SceneNode"]] = None, factor = 10000) -> Tuple[bool, List[Item]]:
    ...
    :param fixed_nodes: List of nods that should not be moved, but should be used when deciding where the others nodes
                        are placed.
    :param factor: The library that we use is int based. This factor defines how acqidite we want it to be.
    ...
    """
    # Logger.log("e",qidi.QIDIApplication.QIDIApplication.getInstance().get_copy_splicing)
    spacing = int(1.5 * factor)  # 1.5mm spacing.
    
    # spacing = int(qidi.QIDIApplication.QIDIApplication.getInstance().get_copy_splicing * factor)
    #factor = int(spacing / 1.5)

    machine_width = build_volume.getWidth()
    machine_depth = build_volume.getDepth()

    build_plate_bounding_box = Box(machine_width * factor+spacing*4, machine_depth * factor+spacing*4)
...
def findNodePlacement_copy(nodes_to_arrange: List["SceneNode"], build_volume: "BuildVolume", fixed_nodes: Optional[List["SceneNode"]] = None, factor = 10000,spacing=1,checked:bool =True) -> Tuple[bool, List[Item]]:
    """
    Find placement for a set of scene nodes, but don't actually move them just yet.
    :param nodes_to_arrange: The list of nodes that need to be moved.
    :param build_volume: The build volume that we want to place the nodes in. It gets size & disallowed areas from this.
...
def arrange(nodes_to_arrange: List["SceneNode"], build_volume: "BuildVolume", fixed_nodes: Optional[List["SceneNode"]] = None, factor = 10000, add_new_nodes_in_scene: bool = False) -> bool:
    ...
    :param fixed_nodes: List of nods that should not be moved, but should be used when deciding where the others nodes
                        are placed.
    :param factor: The library that we use is int based. This factor defines how acqidicte we want it to be.
...
def arrange_copy(nodes_to_arrange: List["SceneNode"], build_volume: "BuildVolume", fixed_nodes: Optional[List["SceneNode"]] = None,spacing=1,checked: bool = True, factor = 10000, add_new_nodes_in_scene: bool = False) -> bool:
    """
    Find placement for a set of scene nodes, and move them by using a single grouped operation.
    :param nodes_to_arrange: The list of nodes that need to be moved.
    :param build_volume: The build volume that we want to place the nodes in. It gets size & disallowed areas from this.
    ...
    """
    scene_root = Application.getInstance().getController().getScene().getRoot()
    found_solution_for_all, node_items = findNodePlacement_copy(nodes_to_arrange, build_volume, fixed_nodes, factor,spacing,checked)
