...
class Arrange:
    """
    The Arrange classed is used together with :py:class:`qidi.Arranging.ShapeArray.ShapeArray`. Use it to find good locations for objects that you try to put
    on a build place. Different priority schemes can be defined so it alters the behavior while using the same logic.

    .. note::

       Make sure the scale is the same between :py:class:`qidi.Arranging.ShapeArray.ShapeArray` objects and the :py:class:`qidi.Arranging.Arrange.Arrange` instance.
    """
    ...
    def create(cls, scene_root = None, fixed_nodes = None, scale = 0.5, x = 350, y = 250, min_offset = 8) -> "Arrange":
        """Helper to create an :py:class:`qidi.Arranging.Arrange.Arrange` instance
    ...
    def bestSpot(self, shape_arr, start_prio = 0, step = 1) -> LocationSuggestion:
        ...
        :param step: Slicing value, higher = more skips = faster but less acqidite
