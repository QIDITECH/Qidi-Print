...
from QD.Job import Job  # For our background task of loading MachineNodes lazily.
from QD.JobQueue import JobQueue  # For our background task of loading MachineNodes lazily.
from QD.Logger import Logger
from QD.Settings.ContainerRegistry import ContainerRegistry  # To listen to containers being added.
from QD.Signal import Signal
import qidi.QIDIApplication  # Imported like this to prevent circular dependencies.
from qidi.Machines.MachineNode import MachineNode
from qidi.Settings.GlobalStack import GlobalStack  # To listen only to global stacks being added.
...
class ContainerTree:
    ...
    def __init__(self) -> None:
        ...
        qidi.QIDIApplication.QIDIApplication.getInstance().initializationFinished.connect(self._onStartupFinished)  # Start the background task to load more machine nodes after start-up is completed.
