...
from qidi.Settings.SettingVisibilityPreset import SettingVisibilityPreset
...
class SettingVisibilityPresetsModel(QObject):
    ...
    def __init__(self, preferences: Preferences, parent = None) -> None:
        ...
        self._preferences.addPreference("qidi/active_setting_visibility_preset", "basic")

        # Preference that stores the "custom" set so it can always be restored (even after a restart)
        self._preferences.addPreference("qidi/custom_visible_settings", basic_visibile_settings)
        self._preferences.preferenceChanged.connect(self._onPreferencesChanged)

        self._active_preset_item = self.getVisibilityPresetById(self._preferences.getValue("qidi/active_setting_visibility_preset"))
    ...
    def _populate(self) -> None:
        ...
        for file_path in Resources.getAllResourcesOfType(QIDIApplication.ResourceTypes.SettingVisibilityPreset):
        ...
        all_setting_visibility_preset.setSettings(list(QIDIApplication.getInstance().getMachineManager().getAllSettingKeys()))
    ...
    def setActivePreset(self, preset_id: str) -> None:
        ...
        if need_to_save_to_custom:
            ...
            if current_visibility_string:
                self._preferences.setValue("qidi/custom_visible_settings", current_visibility_string)

        new_visibility_string = ";".join(preset_item.settings)
        #Logger.log("e",new_visibility_string)
        if preset_id == "custom":
            # Get settings from the stored custom data
            new_visibility_string = self._preferences.getValue("qidi/custom_visible_settings")
        ...
        self._preferences.setValue("qidi/active_setting_visibility_preset", preset_id)
    ...
    def _onPreferencesChanged(self, name: str) -> None:
        ...
        if matching_preset_item is None:
            ...
            if self._active_preset_item is None or self._active_preset_item.presetId == "custom":
                ...
                self._preferences.setValue("qidi/custom_visible_settings", visibility_string)
            else:
        ...
        if self._active_preset_item is None or self._active_preset_item.presetId != item_to_set.presetId:
            ...
            if self._active_preset_item is not None:
                self._preferences.setValue("qidi/active_setting_visibility_preset", self._active_preset_item.presetId)
            self.activePresetChanged.emit()
