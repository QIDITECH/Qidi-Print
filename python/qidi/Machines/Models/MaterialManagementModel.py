...
import qidi.QIDIApplication  # Imported like this to prevent circular imports.
from qidi.Machines.ContainerTree import ContainerTree
from qidi.Settings.QIDIContainerRegistry import QIDIContainerRegistry  # To find the sets of materials belonging to each other, and currently loaded extruder stacks.
...
class MaterialManagementModel(QObject):
    ...
    def removeMaterial(self, material_node: "MaterialNode") -> None:
        ...
        with postponeSignals(container_registry.containerRemoved, compress = CompressTechnique.CompressPerParameterValue):
            # QIDI-6886: Some containers may not have been loaded. If remove one material container, its material file
    ...
    def duplicateMaterialByBaseFile(self, base_file: str, new_base_id: Optional[str] = None,
        ...
        # QIDI-6863: Nodes in ContainerTree will be updated upon ContainerAdded signals, one at a time. It will use the
        ...
        with postponeSignals(container_registry.containerAdded, compress=CompressTechnique.CompressPerParameterValue):
            ...
            favorites_set = set(application.getPreferences().getValue("qidi/favorite_materials").split(";"))
            if base_file in favorites_set:
                favorites_set.add(new_base_id)
                application.getPreferences().setValue("qidi/favorite_materials", ";".join(favorites_set))
    ...
    def createMaterial(self) -> str:
        ...
        new_id = QIDIContainerRegistry.getInstance().uniqueName("custom_material")
    ...
    def addFavorite(self, material_base_file: str) -> None:
        ...
        application = qidi.QIDIApplication.QIDIApplication.getInstance()
        favorites = application.getPreferences().getValue("qidi/favorite_materials").split(";")
        if material_base_file not in favorites:
            favorites.append(material_base_file)
            application.getPreferences().setValue("qidi/favorite_materials", ";".join(favorites))
            application.saveSettings()
