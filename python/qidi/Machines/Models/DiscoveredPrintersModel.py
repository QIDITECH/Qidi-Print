...
from QD.OutputDevice.OutputDeviceManager import ManualDeviceAdditionAttempt

if TYPE_CHECKING:
    from QD.OutputDevice.OutputDevicePlugin import OutputDevicePlugin
    from qidi.QIDIApplication import QIDIApplication
    from qidi.PrinterOutput.NetworkedPrinterOutputDevice import NetworkedPrinterOutputDevice
...
class DiscoveredPrinter(QObject):
    ...
    def _hasHumanReadableMachineTypeName(self, machine_type_name: str) -> bool:
        from qidi.QIDIApplication import QIDIApplication
        results = QIDIApplication.getInstance().getContainerRegistry().findDefinitionContainersMetadata(name = machine_type_name)
    ...
    def _getMachineTypeNameFromId(self, machine_type_id: str) -> str:
        machine_type_name = ""
        from qidi.QIDIApplication import QIDIApplication
        results = QIDIApplication.getInstance().getContainerRegistry().findDefinitionContainersMetadata(id = machine_type_id)
        if results:
...
class DiscoveredPrintersModel(QObject):
    """Discovered printers are all the printers that were found on the network, which provide a more convenient way to
     ...
     that printer to QIDI as the active one).
    """
