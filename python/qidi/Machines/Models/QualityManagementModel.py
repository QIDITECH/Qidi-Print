...
from QD.Settings.InstanceContainer import InstanceContainer  # To create new profiles.

import qidi.QIDIApplication  # Imported this way to prevent circular imports.
from qidi.Settings.ContainerManager import ContainerManager
from qidi.Machines.ContainerTree import ContainerTree
from qidi.Settings.qidi_empty_instance_containers import empty_quality_changes_container
from qidi.Settings.IntentManager import IntentManager
from qidi.Machines.Models.MachineModelUtils import fetchLayerHeight
from qidi.Machines.Models.IntentTranslations import intent_translations
...
class QualityManagementModel(ListModel):
    ...
    def _createQualityChanges(self, quality_type: str, intent_category: Optional[str], new_name: str, machine: "GlobalStack", extruder_stack: Optional["ExtruderStack"]) -> "InstanceContainer":
        ...
        quality_changes.setMetaDataEntry("setting_version", qidi.QIDIApplication.QIDIApplication.getInstance().SettingVersion)
    ...
    def _update(self):
        ...
        for quality_changes_group in quality_changes_group_list:
            # QIDI-6913 Note that custom qualities can be based on "not supported", so the quality group can be None.
    ...
    def getFileNameFilters(self, io_type):
        ...
        #TODO: This function should be in QD.Resources!
        filters = []
        all_types = []
