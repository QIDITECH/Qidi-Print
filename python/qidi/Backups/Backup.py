...
class Backup:
    ...
    IGNORED_FILES = [r"qidi\.log", r"plugins\.json", r"cache", r"__pycache__", r"\.qmlc", r"\.pyc"]
    """These files should be ignored when making a backup."""

    SECRETS_SETTINGS = ["general/qidi_auth_data"]
    """Secret preferences that need to obfuscated when making a backup of QIDI"""

    catalog = i18nCatalog("qidi")
    """Re-use translation catalog"""

    def __init__(self, application: "QIDIApplication", zip_file: bytes = None, meta_data: Dict[str, str] = None) -> None:
    ...
    def makeFromCurrent(self) -> None:
        ...
        qidi_release = self._application.getVersion()
        version_data_dir = Resources.getDataStoragePath()

        Logger.log("d", "Creating backup for QIDI %s, using folder %s", qidi_release, version_data_dir)
    ...
    def restore(self) -> bool:
        ...
        if not self.zip_file or not self.meta_data or not self.meta_data.get("qidi_release", None):
            # We can restore without the minimum required information.
            Logger.log("w", "Tried to restore a QIDI backup without having proper data or meta data.")
            self._showMessage(
                self.catalog.i18nc("@info:backup_failed",
                                   "Tried to restore a QIDI backup without having proper data or meta data."))
            return False

        current_version = self._application.getVersion()
        version_to_restore = self.meta_data.get("qidi_release", "master")

        if current_version < version_to_restore:
            # Cannot restore version newer than current because settings might have changed.
            Logger.log("d", "Tried to restore a QIDI backup of version {version_to_restore} with qidi version {current_version}".format(version_to_restore = version_to_restore, current_version = current_version))
            self._showMessage(
                self.catalog.i18nc("@info:backup_failed",
                                   "Tried to restore a QIDI backup that is higher than the current version."))
            return False
        ...
        try:
            ...
            Logger.log("d", f"The following error occurred while trying to restore a QIDI backup: {str(e)}")
            self._showMessage(self.catalog.i18nc("@info:backup_failed", "The following error occurred while trying to restore a QIDI backup:") + str(e))
            return False
    ...
    def _extractArchive(archive: "ZipFile", target_path: str) -> bool:
        ...
        from qidi.QIDIApplication import QIDIApplication
        config_filename = QIDIApplication.getInstance().getApplicationName() + ".cfg"  # Should be there if valid.
