...
class QIDIConnect(QObject):

    def __init__(self):

        super(QIDIConnect, self).__init__()              #不运行父类初始化，不知道原因，但是只有这样才能让下面的定时器被调用时能正常运行
    ...
    def startPrintForButton(self, m, action):
        ...
        if action == "No":
            ...
            return
        self._last_file_name=self._last_file_name.replace(" ", "\ ")
    ...
    def ip_info_update(self, response):
        ...
        try:
            ...
            mac_address = response['result']['system_info']['network']['eth0']['mac_address']
            ip_address = response['result']['system_info']['network']['eth0']['ip_addresses'][0]['address']
        try:
    ...
    def printer_info_update(self, response):
        ...
        state = response.get('result',{}).get('status',{}).get('print_stats',{}).get('state','standby')
        extruder_tem = response.get('result',{}).get('status',{}).get('extruder',{}).get('temperature','0')
        extruder_tar = response.get('result',{}).get('status',{}).get('extruder',{}).get('target','0')
        heater_bed_tem = response.get('result',{}).get('status',{}).get('heater_bed',{}).get('temperature','0')
        heater_bed_tar = response.get('result',{}).get('status',{}).get('heater_bed',{}).get('target','0')
        homed_axes = response.get('result',{}).get('status',{}).get('toolhead',{}).get('homed_axes','')
        x_location = response.get('result',{}).get('status',{}).get('toolhead',{}).get('position',[0,0,0,0])[0]
        y_location = response.get('result',{}).get('status',{}).get('toolhead',{}).get('position',[0,0,0,0])[1]
        z_location = response.get('result',{}).get('status',{}).get('toolhead',{}).get('position',[0,0,0,0])[2]
        FilaSen ="1" if response.get('result',{}).get('status',{}).get('filament_switch_sensor fila',{}).get('enabled',False) else "0"
        e_location = response.get('result',{}).get('status',{}).get('toolhead',{}).get('position',[0,0,0,0])[3]
        FanSpeed = response.get('result',{}).get('status',{}).get('output_pin fan0',{}).get('value','0')
        rapid_cooling_speed = response.get('result',{}).get('status',{}).get('output_pin fan2',{}).get('value','0')
        chamber_cooling_speed = response.get('result',{}).get('status',{}).get('output_pin fan1',{}).get('value','0')
        print_progress = response.get('result',{}).get('status',{}).get('display_status',{}).get('progress','0')
    ...
    _instance = None
    @classmethod
    def getInstance(cls, *args, **kwargs) -> "QIDIConnect":
        if not QIDIConnect._instance:
            QIDIConnect._instance = cls()
        return QIDIConnect._instance