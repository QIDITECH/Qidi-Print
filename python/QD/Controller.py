...
from QD.Event import Event, KeyEvent, MouseEvent, ToolEvent, ViewEvent
...
class Controller:
    ...
    def __init__(self, application: "Application") -> None:
        ...
        self._default_pass = None
        
        self._button_pass = None
    ...
    def setActiveTool(self, tool: Optional[Union["Tool", str]]):
        ...
        from QD.Scene.Selection import Selection  # Imported here to prevent a circular dependency.
    ...
    def event(self, event: Event):
        ...
        if event.type == Event.MouseReleaseEvent:
            ...
            if MouseEvent.LeftButton in event.buttons:
                px = event.x
                py = event.y
                if not self._button_pass:
                    self._button_pass =  self._application.getRenderer().getRenderPass("button")
                get_id=self._button_pass.getIdAtPosition(px, py)
                if get_id:
                    if get_id == "open" :
                        ...
                        self.copyfileRequested.emit()
                        return
                    elif get_id == "flat" :
                        self.flatfileRequested.emit()
                        return
                    elif get_id == "place" :
                        self.placefileRequested.emit()
                        return
        ...
        if event.type == Event.MouseMoveEvent :
                if not self._button_pass:
                    self._button_pass =  self._application.getRenderer().getRenderPass("button")
                get_id=self._button_pass.getIdAtPosition(event.x, event.y)
                if get_id == "open" :
                    self._event = event
                    self.deletedislightRequested.emit()
                    self.flatdislightRequested.emit()
                    self.placedislightRequested.emit()
                    self.copydislightRequested.emit()
                    self.openlightRequested.emit()

                elif get_id == "delete" :
                    self._event = event
                    self.opendislightRequested.emit()
                    self.flatdislightRequested.emit()
                    self.placedislightRequested.emit()
                    self.copydislightRequested.emit()
                    self.deletelightRequested.emit()
                elif get_id == "copy" :
                    self._event = event
                    self.opendislightRequested.emit()
                    self.deletedislightRequested.emit()
    ...
    flatfileRequested = Signal()
    placefileRequested = Signal()
    copyfileRequested = Signal()

    openlightRequested = Signal()
    deletelightRequested = Signal()
    flatlightRequested = Signal()
    placelightRequested = Signal()
    copylightRequested = Signal()

    opendislightRequested = Signal()
    deletedislightRequested = Signal()
    flatdislightRequested = Signal()
    placedislightRequested = Signal()
    copydislightRequested = Signal()
