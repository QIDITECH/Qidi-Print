...
from QD.Mesh.MeshData import MeshType
from QD.Mesh.MeshData import calculateNormalsFromVertices
from QD.Mesh.MeshData import calculateNormalsFromIndexedVertices
...
class MeshBuilder:
    ...
    def setVertexColor(self, index, color):
        ...
        :param color: :type{QD.Math.Color} the color of the vertex.
        """
    ...
    def createCube(self, verts, color=None):
        start = self.getVertexCount()
        verts = [verts[0], verts[1], verts[2], verts[3],
                 ...
                 verts[7], verts[3], verts[2], verts[6]
                 ]
        verts = numpy.asarray(verts, dtype=numpy.float32)
        self.addVertices(verts)
        indices = []
        for i in range(0, 24, 4):  # All 6 quads (12 triangles)
            indices.append([start+i, start+i + 2, start+i + 1])
            indices.append([start+i, start+i + 3, start+i + 2])
    ...
    def createCylinder(self, size, nb, height, color=None):
        start = self.getVertexCount()
        r = size / 2
        sup = size * 0.1
        l = -height
        ...
        for i in range(0, tot, 3):  #
            indices.append([start+i, start+i + 1, start+i + 2])
        indices=numpy.asarray(indices, dtype=numpy.int32)
        self.addIndices(indices)
        if color: #If we have a colour, add a colour to all of the vertices.
            vertex_count = self.getVertexCount()
