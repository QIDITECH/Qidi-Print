...
from QD.MimeTypeDatabase import MimeTypeDatabase, MimeType #To register the MIME type of the preference file.
from QD.SaveFile import SaveFile
from QD.Signal import Signal, signalemitter

MimeTypeDatabase.addMimeType(
    MimeType(
        name = "application/x-qdtech-preferences",
        comment = "QDTECH Preferences File",
        suffixes = ["cfg"],
...
class Preferences:
    ...
    def _findPreference(self, key: str) -> Optional[Any]:
        ...
        if self._preferences =={}:
            try:
                preferences_filename = Resources.getPath(Resources.Preferences,"QIDIPrint.cfg")
                self.readFromFile(preferences_filename)
            except:
                pass
