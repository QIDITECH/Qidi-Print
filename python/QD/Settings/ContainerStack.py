...
from QD.Settings.DefinitionContainer import DefinitionContainer #For getting all definitions in this stack.
...
MimeTypeDatabase.addMimeType(
    MimeType(
        name = "application/x-qdtech-containerstack",
        comment = "QDTECH Container Stack",
        suffixes = [ "stack.cfg" ]
    )
)
...
MimeTypeDatabase.addMimeType(
    MimeType(
        name = "application/x-qdtech-extruderstack",
        comment = "QDTECH Extruder Stack",
        suffixes = [ "stack.cfg" ]
    )
)
...
class ContainerStack(QObject, ContainerInterface, PluginObject):
    ...
    def _readAndValidateSerialized(cls, serialized: str) -> configparser.ConfigParser:
        ...
        the serialized data. Due to legacy problem, those data may not be available if it comes from an ancient QIDI.
        """
