...
from QD.FastConfigParser import FastConfigParser
...
from QD.Settings.PropertyEvaluationContext import PropertyEvaluationContext #For typing.
...
from QD.Settings.Interfaces import ContainerInterface, ContainerRegistryInterface
...
MimeTypeDatabase.addMimeType(
    MimeType(
        name = "application/x-qdtech-instancecontainer",
        comment = "QDTECH Instance Container",
        suffixes = [ "inst.cfg" ]
    )
)
...
class InstanceContainer(QObject, ContainerInterface, PluginObject):
    ...
    # QIDI-3497, the nameChanged signal should be changed to a pyqtSignal. However,
    # pyqtSignal throws TypeError when calling disconnect() when there are no connections.
    # This causes a lot of errors in QIDI code when we try to disconnect from nameChanged.
    # Therefore, rather than change the type of nameChanged, we add an extra signal that
    # is used as notify for the property.
    #
    # TODO: Remove this once the QIDI code has been refactored to not use nameChanged anymore.
    ...
    def setProperty(self, key: str, property_name: str, property_value: Any, container: ContainerInterface = None, set_from_cache: bool = False) -> None:
        ...
        if key not in self._instances:
            ...
            instance = SettingInstance(setting_definition[0], self)
            instance.propertyChanged.connect(self.propertyChanged)
            #Logger.log("e",instance.definition.key)
            #Logger.log("e",instance.definition.value)
            self._instances[instance.definition.key] = instance
    ...
    def serializetest(self) -> Dict[str, Any]:
        #for key, instance in sorted(self._instances.items()):
        #   try:
        #         test= str(instance.value)
        #   except AttributeError:
        #        pass
