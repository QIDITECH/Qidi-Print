...
from QD.Decorators import deprecated
from QD.FlameProfiler import pyqtSlot

from QD.Logger import Logger
from QD.Settings import SettingRelation
...
class SettingDefinitionsModel(QAbstractListModel):
    ...
    def getIndex(self, key: str) -> int:
        ...
        #Logger.log("e",self._row_index_list)
        try:
    ...
    def getkey(self, index: int) -> str:
        ...
        try:
            return str(self._definition_list[self._row_index_list[index]]).split("key=")[1].split(" container")[0]
        except :
            return ""
        ...
        if self._filter_dict and not definition.matchesFilter(**filter):
            if self._show_ancestors:
                if self._isAnyDescendantFiltered(definition):
                    Application.getInstance()._preferences.setValue("qidi/show_search",Application.getInstance()._preferences.getValue("qidi/show_search")+'/'+definition.key)
                    return True
            return False
