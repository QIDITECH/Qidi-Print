...
class ContainerQuery:
    ...
    def execute(self, candidates: Optional[List[Any]] = None) -> None:
        ...
        # are looking for all materials for QD3+AA0.4 and later for QD3+AA0.8,
        # then for QD3+AA0.4 we'll first filter on QD3 and then for AA0.4, so
        # that we can re-use the cached query for QD3 when we look for
        # QD3+AA0.8.
