...
from QD.Backend.SignalSocket import SignalSocket
...
class Backend(PluginObject):
    ...
    def __init__(self):
        ...
        QD.Application.Application.getInstance().callLater(self._createSocket)
    ...
    def getEngineCommand(self):
        ...
        return [QD.Application.Application.getInstance().getPreferences().getValue("backend/location"), "--port", str(self._socket.getPort())]
    ...
    def _onSocketStateChanged(self, state):
        ...
        if state == Arcus.SocketState.Listening:
            if not QD.Application.Application.getInstance().getUseExternalBackend():
                self.startEngine()
    ...
    def _logSocketState(self, state):
        """Debug function created to provide more info for QIDI-2127"""
    ...
    def _createSocket(self, protocol_file):
        ...
        if not self._socket.registerAllMessageTypes(protocol_file):
            Logger.log("e", "Could not register QDTECH protocol messages: %s", self._socket.getLastError())

        if QD.Application.Application.getInstance().getUseExternalBackend():
