...
class Theme(QObject):
    def __init__(self, engine, parent = None) -> None:
        ...
        #Preferences.getInstance().addPreference('qidi/size', 1)
        ...
        # will be returned. We cache the initial language here is because QIDI can only change its language if it gets
        # restarted, so we need to keep the fonts consistent in a single QIDI run.
        self._preferences = QD.Application.Application.getInstance().getPreferences()
    ...
    def reload(self):
        ...
        self._icons = {}
        self._images = {}
        application = QD.Application.Application.getInstance()
    ...
    def getmaterColor(self,colorfor:str) -> str:
        global_container_stack = QD.Application.Application.getInstance().getGlobalContainerStack()
        if global_container_stack:
            machine_extruder_count = global_container_stack.getProperty("machine_extruder_count", "value")
            defaultColors = ["#ffc924", "#86ec21", "#22eeee", "#245bff", "#9124ff", "#ff24c8"]
            if machine_extruder_count ==1 :
                default_color = defaultColors[0]
                extruder =QD.Application.Application.getInstance().getExtruderManager().getActiveExtruderStack()
    ...
    #        return self._sizes[size] * float(self._preferences.getValue("qidi/size"))
    #    Logger.log('w', 'No size %s defined in Theme', size)
    #    return QSizeF() * float(self._preferences.getValue("qidi/size"))

    @pyqtSlot(str, result = "QSizeF")
    def getSize(self, size) -> QSizeF:
        if size in self._sizes:
            if size == "default_lining":
                return self._sizes[size] 
            else:
                #return self._sizes[size] *0.8
                #Logger.log("e", "size = %s", size)
                #if self._preferences.getValue("qidi/imagesize"):
                #    return self._sizes[size] * float(self._preferences.getValue("qidi/imagesize") )
                #else:
                #    self._preferences.addPreference("qidi/imagesize", 1)
                return self._sizes[size] * float(self._preferences.getValue("qidi/imagesize") if self._preferences.getValue("qidi/imagesize") else 1)
        Logger.log("w", "No size %s defined in Theme", size)
        #return QSizeF()
        #Logger.log("e",self._preferences.getValue("qidi/fontsize"))
        return QSizeF() * float(self._preferences.getValue("qidi/imagesize") if self._preferences.getValue("qidi/imagesize") else 1)
    ...
    def load(self, path: str, is_first_call: bool = True) -> None:
        ...
        if "fonts" in data:
            ...
            for name, font in data["fonts"].items():
                ...
                #if self._preferences.getValue("qidi/fontsize") :
                #    q_font.setPointSize(int(font.get("size", 1) * system_font_size * float(self._preferences.getValue("qidi/fontsize") if self._preferences.getValue("qidi/fontsize") else 1) ))
                #else :
                #    self._preferences.addPreference("qidi/fontsize", 1)
                q_font.setPointSize(int(font.get("size", 1) * system_font_size * float(self._preferences.getValue("qidi/fontsize") if self._preferences.getValue("qidi/fontsize") else 1) ))
