...
from QD.Backend.Backend import Backend #For typing.
from QD.ConfigurationErrorMessage import ConfigurationErrorMessage
from QD.FileHandler.ReadFileJob import ReadFileJob
from QD.FileHandler.WriteFileJob import WriteFileJob
from QD.Mesh.MeshFileHandler import MeshFileHandler
from QD.Qt.Bindings.Theme import Theme
from QD.Workspace.WorkspaceFileHandler import WorkspaceFileHandler
from QD.Application import Application
from QD.PackageManager import PackageManager #For typing.
from QD.Qt.QtRenderer import QtRenderer
from QD.Qt.Bindings.Bindings import Bindings
from QD.Qt.Bindings.MainWindow import MainWindow #For typing.
...
from QD.Job import Job #For typing.
from QD.JobQueue import JobQueue
...
from QD.Operations.GroupedOperation import GroupedOperation #To clear the scene.
from QD.Operations.RemoveSceneNodeOperation import RemoveSceneNodeOperation #To clear the scene.
from QD.Scene.Iterator.DepthFirstIterator import DepthFirstIterator #To clear the scene.
from QD.Scene.SceneNode import SceneNode #To clear the scene.
from QD.Scene.Selection import Selection #To clear the selection after clearing the scene.

import QD.Settings.InstanceContainer  # For version upgrade to know the version number.
import QD.Settings.ContainerStack  # For version upgrade to know the version number.
import QD.Preferences  # For version upgrade to know the version number.
from QD.Mesh.ReadMeshJob import ReadMeshJob
from QD.Qt.Duration import Duration
from QD.Qt.Duration import DurationFormat


import QD.Qt.Bindings.Theme
...
class QtApplication(QApplication, Application):
    ...
    def __init__(self, tray_icon_name: str = None, **kwargs) -> None:
        ...
        self._gcode_time = Duration(None)
        ...
        self.setOrganizationDomain("https://qidi.com/")
        self.setOrganizationName("QIDI B.V.")
...
#######
        ...
        if Platform.isWindows():
            searchDir = searchDir + '\\examples'
        elif Platform.isOSX():
            searchDir = searchDir + '/examples'
        elif Platform.isLinux():
            searchDir =os.path.join(os.path.dirname(os.path.realpath(__file__)).split("/lib")[0],"resources") + "/examples"
        Logger.log("i", "example dir: %s", searchDir)
        for root, dirs, _files in os.walk(searchDir):
            for name in _files:
                Logger.log("i", "example file: %s", name)
                files.append(os.path.join(searchDir,name))
        #print("files",files)
        for f in files:
            self._example_files.append(QUrl.fromLocalFile(f))
...
######
    ...
    def _onJobFinished(self, job: Job) -> None:
        ...
        if isinstance(job, (ReadMeshJob, ReadFileJob, WriteFileJob)) and job.getAddToRecentFiles():
            ...
            if job.getFileName().endswith(".gcode") :
                with open(job.getFileName(), "r", encoding = "utf-8") as file:
                    file_line = file.readlines()
                    
                self._gcode_time.setDuration(0)
                if len(file_line) > 30 :
                    for i in range(-1,-30,-1):
                        if file_line[i].find("TIME_ELAPSED") != -1:
                            self._gcode_time.setDuration(float(file_line[i].split("TIME_ELAPSED:")[1]))
                            self.gcodePrintTimeChanged.emit()
                            break
                #if file_line.find("Recording time : ")!=-1:
                #    self._gcode_time.setDuration(float(file_line.split("Recording time : ")[1]))
                #else:
                #    self._gcode_time.setDuration(0)
                #self.gcodePrintTimeChanged.emit()
                 

    gcodePrintTimeChanged = pyqtSignal()


    @pyqtProperty(Duration, notify = gcodePrintTimeChanged)
    def gcodePrintTime(self) -> Duration:
        return self._gcode_time
    ...
    def getTheme(self) -> Optional[Theme]:
        if self._theme is None:
            ...
            self._theme = QD.Qt.Bindings.Theme.Theme.getInstance(self._qml_engine)
        return self._theme
    ...
    def createQmlComponent(self, qml_file_path: str, context_properties: Dict[str, "QObject"] = None) -> Optional["QObject"]:
        ...
        if self._qml_engine is None: # Protect in case the engine was not initialized yet
            return None
        if ("qrc" in qml_file_path ) :
            path= QUrl(qml_file_path)
        else:
    ...
    def _screenScaleFactor(self) -> float:
        ...
        if sys.platform == "darwin":
            ...
            # determine a device pixel ratio from font metrics, using the same logic as QD.Theme
