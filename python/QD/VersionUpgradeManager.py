...
import QD.Message  # To show the "upgrade succeeded" message.
import QD.MimeTypeDatabase  # To know how to save the resulting files.
import QD.i18n  # To translate the "upgrade succeeded" message.
from QD.Application import Application
from QD.Logger import Logger
from QD.MimeTypeDatabase import MimeType
from QD.PluginObject import PluginObject
from QD.PluginRegistry import PluginRegistry  # To find plug-ins.
from QD.Resources import Resources  # To load old versions from.

from PyQt5.QtCore import QCoreApplication

from QD.SaveFile import SaveFile

catalogue = QD.i18n.i18nCatalog("qdtech")
...
class VersionUpgradeManager:
    ...
    def upgrade(self) -> bool:
        ...
        if upgraded:
            message = QD.Message.Message(text = catalogue.i18nc("@info:version-upgrade", "A configuration from an older version of {0} was imported.", Application.getInstance().getApplicationName()), title = catalogue.i18nc("@info:title", "Version Upgrade"))
            message.show()
    ...
    def _upgradeFile(self, storage_path_absolute: str, configuration_file: str, old_configuration_type: str) -> bool:
        ...
        try:
            mime_type = QD.MimeTypeDatabase.MimeTypeDatabase.getMimeTypeForFile(configuration_file)
        except QD.MimeTypeDatabase.MimeTypeNotFoundError:
            return False
        ...
        if version != old_version or configuration_type != old_configuration_type:
            ...
            mime_type = QD.MimeTypeDatabase.MimeTypeDatabase.getMimeType(mime_type_name)  # Get the actual MIME type object, from the name.
    ...
    def updateFilesData(self, configuration_type: str, version: int, files_data: List[str], file_names_without_extension: List[str]) -> Optional[FilesDataUpdateResult]:
        ...
        #Logger.log("e",configuration_type)
        #Logger.log("e",version)
