...
from QD.MimeTypeDatabase import MimeTypeDatabase  # To get the type of container we're loading.
from QD.Resources import Resources
from QD.Signal import Signal
from QD.Version import Version as QDVersion
...
class PackageManager(QObject):
    ...
    def __init__(self, application: "QtApplication", parent: Optional[QObject] = None) -> None:
        ...
        self._available_package_versions = {}  # type: Dict[str, Set[QDVersion]]
    ...
    def addAvailablePackageVersion(self, package_id: str, version: "QDVersion") -> None:
    ...
    def isPackageCompatible(self, package_api_version: QDVersion) -> bool:
        """
    ...
    def checkIfPackageCanUpdate(self, package_id: str) -> bool:
        ...
        if bundled_package_dict is not None:
            current_version = QDVersion(bundled_package_dict["package_info"]["package_version"])

        installed_package_dict = self._installed_package_dict.get(package_id)
        if installed_package_dict is not None:
            current_version = QDVersion(installed_package_dict["package_info"]["package_version"])
    ...
    def _shouldInstallCandidate(self, candidate_dict: Dict[str, Any], base_dict: Dict[str, Any]) -> bool:
        ...
        sdk_version_candidate = QDVersion(candidate_dict["sdk_version"])
        ...
        version_candidate = QDVersion(candidate_dict["package_version"])
        version_base = QDVersion(base_dict["package_version"])
    ...
    def reEvaluateDismissedPackages(self, subscribed_packages_payload: List[Dict[str, Any]], sdk_version: str) -> None:
        ...
        against our current QIDI SDK version, and if it is in there - we remove
        the already dismissed package from the above mentioned list.
        :param subscribed_packages_payload: The response from Web QIDI, a list
        of packages that a user is subscribed to.
        :param sdk_version: Current QIDI SDK version.
        """
    ...
    def installPackage(self, filename: str) -> Optional[str]:
        ...
        try:
            ...
            # We do not check if the same package has been installed already here because, for example, in QIDI,
            ...
            with self._container_registry.lockFile():
                ...
                package_cache_dir = os.path.join(os.path.abspath(Resources.getCacheStoragePath()), "qidi_packages")
                if not os.path.exists(package_cache_dir):
                    os.makedirs(package_cache_dir, exist_ok=True)

                target_file_path = os.path.join(package_cache_dir, package_id + ".qidipackage")
