...
import QD.Application  # Circular dependency blah
from QD.Controller import Controller
from QD.Event import Event, MouseEvent
from QD.Math.Plane import Plane #Typing for drag plane.
from QD.Math.Vector import Vector #Typing for drag coordinates.
...
class Tool(PluginObject):
    ...
    def __init__(self) -> None:
        super().__init__()
        self._controller = QD.Application.Application.getInstance().getController()  # Circular dependency blah
        self._enabled = True
    ...
    def event(self, event: Event) -> bool:
        ...
        if not self._selection_pass:
            self._selection_pass = cast(SelectionPass, QD.Application.Application.getInstance().getRenderer().getRenderPass("selection"))
            if not self._selection_pass:
                return False
