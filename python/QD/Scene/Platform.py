...
class Platform(SceneNode.SceneNode):
    ...
    def _onPlatformLoaded(self, job):
        ...
        if node.getMeshData():
            ...
            # path = r"G:\QIDIWrite\QIDI\build\package\resources\meshes\tubiao.STL"
            # reader = self._mesh_handler.getReaderForFile(r"G:\QIDIWrite\QIDI\build\package\resources\meshes\tubiao.STL")
            # self.setMeshData(reader.read(r"G:\QIDIWrite\QIDI\build\package\resources\meshes\tubiao.STL").getMeshData())
