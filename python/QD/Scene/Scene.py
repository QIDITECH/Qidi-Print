...
from QD.Mesh.ReadMeshJob import ReadMeshJob  # To reload a mesh when its file was changed.
from QD.Message import Message  # To display a message for reloading files that were changed.
...
class Scene:
    ...
    def addWatchedFile(self, file_path: str) -> None:
        ...
        # File watcher causes qidi to crash on windows if threaded from removable device (usb, ...). Create QEventLoop earlier to fix this.
        if Platform.isWindows():
            QEventLoop()
    ...
    def _onFileChanged(self, file_path: str) -> None:
        ...
        from QD.Scene.Iterator.DepthFirstIterator import DepthFirstIterator  # To find which nodes to reload when files have changed.
