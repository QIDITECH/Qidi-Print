...
from cura.MyWifiSend import MyWifiSend
import win32api
import trimesh
...
class CuraApplication(QtApplication):
    ...
    def __init__(self, **kwargs):
        ...
        preferences.addPreference("cura/dialog_on_project_save", False)
        ...
        preferences.addPreference("general/input_ip", "Please input host IP.")
        preferences.addPreference("general/iplist", "")
        preferences.addPreference("view/switch_layer_view", True)
        preferences.addPreference("color/extruder1", "#bb1e10")
        preferences.addPreference("color/extruder2", "#ffc924")
        preferences.addPreference("view/show_list_of_objects", False)
...
###########
        ...
        defaultMachineName = [DEFAULT_MACHINE_QIDI1, DEFAULT_MACHINE_X_CF_PRO, DEFAULT_MACHINE_X_ONE2, DEFAULT_MACHINE_X_SMART, DEFAULT_MACHINE_X_PRO,DEFAULT_MACHINE_X_MAX,
                              DEFAULT_MACHINE_X_PLUS, DEFAULT_MACHINE_X_MAKER, DEFAULT_MACHINE_I_MATE, DEFAULT_MACHINE_I_FAST, DEFAULT_MACHINE_I_MATE_S]
        defaultMachineType = ['qidi1', 'x-cf_pro', 'x-one2', 'x-smart', 'x-pro', 'x-max', 'x-plus', 'x-maker', 'i-mate', 'i-fast', 'i-mate_s']
        
        while (container_stacks and len(container_stacks) > 0):  # 删除掉非内部定义的机器
            deleteMachine = False
            for i in range(len(container_stacks)):
                if ((container_stacks[i].name not in defaultMachineName) or (container_stacks[i].id not in defaultMachineName)):
                    os.remove(container_stacks[i]._path)
                    container_stacks.pop(i)
                    deleteMachine = True
                    break
            if not deleteMachine:
                break
        for i in range(len(defaultMachineName)):                        #添加未加入的机型
            machineName = defaultMachineName[i]
            addMachineFlag = True
            for container in container_stacks:
                if machineName == container.name:
                    addMachineFlag = False
                    break
            if(addMachineFlag):
                self.getMachineManager().addMachine(machineName, defaultMachineType[i])
                print("add new machine ",machineName)
###############
    ...
    def registerObjects(self, engine):
        ...
        qmlRegisterSingletonType(MyWifiSend, "Cura", 1, 0, "MyWifiSend", MyWifiSend.getInstance)
        MyWifiSend.getInstance()                #提前初始化，好让扫描设备的动作先运行
...
#        print(Resources.getPath(Resources.Preferences))
#        print(Resources.getPath(Resources.Cache))
#        Resources.factoryReset()
#        self.ignoreSaveCfg = True
...
########控制面板
    ...
    @pyqtSlot(bool)
    def installCP(self, user_choice):
        if user_choice:
            exePath = os.path.join(os.path.dirname(sys.argv[0]), 'QIDI CP Setup.exe')
            if not os.path.exists(exePath):
                exePath = ".\\QIDI CP Setup.exe"
            Logger.log("i", exePath)
            win32api.ShellExecute(0, 'open', exePath, '', '', 1)
            self._install_cp_window.hide()
        else:
            self._install_cp_window.hide()
    ...
    def showControlPanel(self):
        if Platform.isWindows():
            if not os.path.exists(os.getenv("APPDATA") + '\QIDIPrint\QIDI Control Panel\QiDi.exe'):
                path = os.path.join(os.path.dirname(sys.argv[0]), 'resources') + '\\qml\InstallCPDialog.qml'
                Logger.log("i", path)
                self._install_cp_window = Application.getInstance().createQmlComponent(path, {"manager": self})
                self._install_cp_window.show()
                return
        
            self.exePath = os.getenv("APPDATA")
            if not os.path.exists(self.exePath):
                self.exePath = ".\\QiDi.exe"
            Logger.log("i", self.exePath + '\QIDIPrint\QIDI Control Panel')
        win32api.ShellExecute(0, 'open', self.exePath + '\QIDIPrint\QIDI Control Panel\QiDi.exe', '', self.exePath + '\QIDIPrint\QIDI Control Panel', 1)

    def modifyArg(self, str_arg, str_modify):
        file_object = open(self.exePath + '\QIDIPrint\QIDI Control Panel\image\config.txt', 'r')
        file_content = file_object.read()
        file_object.close()
        str_primary = self.getArgFromFile(str_arg)
        file_content = file_content.replace(str_arg + ":" + str_primary, str_arg + ":" + str_modify)
        file_object = open(self.exePath + '\QIDIPrint\QIDI Control Panel\image\config.txt', 'w')
        file_object.write(file_content)
        file_object.close()
        
    def getArgFromFile(self, str_arg):
        str_arg = str(str_arg)
        file_object = open(self.exePath + '\QIDIPrint\QIDI Control Panel\image\config.txt','r')
        str_read = file_object.read()
        file_object.close()
        index = str_read.find(str_arg + ":")
        if index != -1:
            str_read = str_read[index + len(str_arg) + 1:]
            arr_read = str_read.split('\n')
            str_read = arr_read[0]
            index = str_read.find("#")
            if index != -1:
                str_args = str_read[:index]
