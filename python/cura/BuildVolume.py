...
class BuildVolume(SceneNode):
    ...
    def render(self, renderer):
        ...
        if not self._shader:
            ...
            self._grid_shader.setUniformValue("u_gridColor0", Color(*theme.getColor("color20").getRgb()))
            self._grid_shader.setUniformValue("u_gridColor1", Color(*theme.getColor("color2").getRgb()))
    ...
    def rebuild(self):
        ...
        if not self._volume_outline_color:
            theme = Application.getInstance().getTheme()
            self._volume_outline_color = Color(*theme.getColor("color16").getRgb())
        ...
        if self._shape != "elliptic":
            ...
            k = self._width / 50
            mb.addLine(Vector(k, min_h, max_d), Vector(0, min_h, max_d + k), color = self._volume_outline_color)
            mb.addLine(Vector(-k, min_h, max_d), Vector(0, min_h, max_d + k), color = self._volume_outline_color)
