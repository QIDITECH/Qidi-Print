...
class CrashHandler:
    ...
    def _createEarlyCrashDialog(self):
        ...
        dialog.setWindowTitle(catalog.i18nc("@title:window", "Qidi Print can't start"))
        dialog.finished.connect(self._closeEarlyCrashDialog)

        layout = QVBoxLayout(dialog)

        label = QLabel()
        label.setText(catalog.i18nc("@label crash message", """<p><b>Oops, Qidi Print has encountered something that doesn't seem right.</p></b>
...
#        self._send_report_checkbox.setChecked(True)
    ...
    def _informationWidget(self):
        ...
        try:
            ...
            self.cura_version = catalog.i18nc("@label unknown version of Qidi", "Unknown")

        crash_info = "<b>" + catalog.i18nc("@label Qidi version number", "Qidi version") + ":</b> " + str(self.cura_version) + "<br/>"
