...
class RenderBatch():
    ...
    def render(self, camera):
        ...
        if self._render_type == self.RenderType.Solid:
            ...
            self._gl.glEnable(self._gl.GL_LINE_SMOOTH)                 #让框的实线变光滑，原来是有锯齿，interl内的gpu这个不管用
            self._gl.glHint(self._gl.GL_LINE_SMOOTH_HINT, self._gl.GL_NICEST);

            # self._gl.glHint(self._gl.GL_POLYGON_SMOOTH_HINT, self._gl.GL_NICEST);
            # self._gl.glEnable(self._gl.GL_POLYGON_SMOOTH)                  #
            # self._gl.glEnable(self._gl.GL_MULTISAMPLE);

            self._gl.glBlendFunc(self._gl.GL_SRC_ALPHA, self._gl.GL_ONE_MINUS_SRC_ALPHA); # 这两行可以解决intel的线不直的问题
            self._gl.glEnable(self._gl.GL_BLEND);
#            self._gl.glEnable(self._gl.GL_POINT_SMOOTH)
#            self._gl.glHint(self._gl.GL_POINT_SMOOTH_HINT, self._gl.GL_NICEST)
#            self._gl.glEnable(self._gl.GL_LINE_SMOOTH)
#            self._gl.glHint(self._gl.GL_LINE_SMOOTH_HINT, self._gl.GL_NICEST)
            self._gl.glEnable(self._gl.GL_BLEND)
