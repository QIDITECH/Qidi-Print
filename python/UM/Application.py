...
class Application:
    ...
    def __init__(self, name: str, version: str, build_type: str = "", is_debug_mode: bool = False, parser: argparse.ArgumentParser = None, parsed_command_line: Dict[str, Any] = None, **kwargs) -> None:
        ...
        self._application_name = "qidi" #type: str
...
#######初次启动时检测系统语言
        if Preferences.getInstance().getValue("general/accepted_user_agreement") is None:
            locale_lang = locale.getdefaultlocale()
            Preferences.getInstance().setValue("general/language", locale_lang[0])
####
