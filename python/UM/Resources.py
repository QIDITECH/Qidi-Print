...
class Resources:
    ...
    def _getPossibleConfigStorageRootPathList(cls):
        ...
        config_root_list = [os.path.join(n, "QIDIPrint") for n in config_root_list]
    ...
    def _getPossibleDataStorageRootPathList(cls) -> List[str]:
        ...
        # Returns all possible root paths for storing app configurations (in old and new versions)
        if Platform.isLinux():
            data_root_list.append(os.path.join(Resources._getDataStorageRootPath(), "QIDIPrint"))
        else:
    ...
    def __initializeStoragePaths(cls):
        ...
        if cls.ApplicationVersion == "master" or cls.ApplicationVersion == "unknown":
            storage_dir_name = os.path.join("QIDIPrint", cls.ApplicationVersion)
        else:
            from UM.Version import Version
            version = Version(cls.ApplicationVersion)
            storage_dir_name = os.path.join("QIDIPrint", "%s.%s" % (version.getMajor(), version.getMinor()))
