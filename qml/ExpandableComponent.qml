...
Item
{
    ...
    property color contentBackgroundColor: QD.Theme.getColor("action_button")

    property color headerBackgroundColor: QD.Theme.getColor("action_button")
    property color headerActiveColor: QD.Theme.getColor("secondary")
    property color headerHoverColor: QD.Theme.getColor("action_button_hovered")
    ...
    property var contentSpacingY: QD.Theme.getSize("narrow_margin").width
    ...
    Binding
    {
        ...
        value:
        {
            return base.enabled ? (expanded ? headerActiveColor : headerBackgroundColor) : QD.Theme.getColor("disabled")
        }
    }
    ...
    RoundedRectangle
    {
        ...
        color: base.enabled ? (base.expanded ? headerActiveColor : headerBackgroundColor) : QD.Theme.getColor("disabled")
        anchors.fill: parent
        ...
        MouseArea
        {
            ...
            onExited: background.color = base.enabled ? (base.expanded ? headerActiveColor : headerBackgroundColor) : QD.Theme.getColor("disabled")
        }
    }
    ...
    QIDI.RoundedRectangle
    {
        ...
        y: dragPreferencesNamePrefix === "" ? (background.height + base.shadowOffset + base.contentSpacingY) : QD.Preferences.getValue(dragPreferencesNamePrefix + dragPreferencesNameY)

        // Make the content aligned with the rest, using the property contentAlignment to decide whether is right or left.
        // In case of right alignment, the 3x padding is due to left, right and padding between the button & text.
        x: dragPreferencesNamePrefix === "" ? (contentAlignment == ExpandableComponent.ContentAlignment.AlignRight ? -width + collapseButton.width + headerItemLoader.width + 3 * background.padding : 0) : QD.Preferences.getValue(dragPreferencesNamePrefix + dragPreferencesNameX)
        ...
        {
            var margin = QD.Theme.getSize("narrow_margin");
            var minPt = base.mapFromItem(null, margin.width, margin.height);
            var maxPt = base.mapFromItem(null,
                QIDIApplication.appWidth() - (contentContainer.width + margin.width),
                QIDIApplication.appHeight() - (contentContainer.height + margin.height));
            ...
            {
                QD.Preferences.setValue(dragPreferencesNamePrefix + dragPreferencesNameX, contentContainer.x);
                QD.Preferences.setValue(dragPreferencesNamePrefix + dragPreferencesNameY, contentContainer.y);
            }
        }
