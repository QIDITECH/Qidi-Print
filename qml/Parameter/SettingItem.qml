...
Item {
    ...
    Label
    {
        ...
        anchors.leftMargin: Math.round(33 * UM.Theme.getSize("default_margin").width/10 + ((definition.depth - 1 * UM.Theme.getSize("default_margin").width/10) * UM.Theme.getSize("setting_control_depth_margin").width))
        ...
        color:
        {
            if (!Cura.ExtruderManager.activeExtruderStackId && machineExtruderCount.properties.value > 1)
            {
                ...
                return model.type == "category" ? UM.Theme.getColor("color4") : UM.Theme.getColor("color8")
            }
            return provider.properties.enabled == "True" ? UM.Theme.getColor("color4") : UM.Theme.getColor("color8")
        }
        font: UM.Theme.getFont("font1")//base.doQualityUserSettingEmphasis && base.stackLevel != undefined && base.stackLevel <= 1 ? UM.Theme.getFont("default_italic") : UM.Theme.getFont("default")
    }
