...
QD.Dialog
{
    ...
    width: QD.Theme.getSize("small_popup_dialog").width
    height: QD.Theme.getSize("small_popup_dialog").height
    ...
    function loadProjectFile() {
        ...
        if (rememberChoiceCheckBox.checked) {
            QD.Preferences.setValue("qidi/choice_on_open_project", "open_as_project")
        }

        QD.WorkspaceFileHandler.readLocalFile(base.fileUrl, base.addToRecent);
    ...
    function loadModelFiles() {
        // update preference
        if (rememberChoiceCheckBox.checked) {
            QD.Preferences.setValue("qidi/choice_on_open_project", "open_as_model")
        }

        QIDIApplication.readLocalFile(base.fileUrl, "open_as_model", base.addToRecent)

        base.hide()
    }

    // override QD.Dialog accept
    function accept () {
        var openAsPreference = QD.Preferences.getValue("qidi/choice_on_open_project")
    ...
    onVisibleChanged: {
        if (visible) {
            var rememberMyChoice = QD.Preferences.getValue("qidi/choice_on_open_project") != "always_ask";
    ...
    Column
    {
        ...
        {
            id: questionText
            text: catalog.i18nc("@text:window", "This is a QIDI project file. Would you like to open it as a project or import the models from it?")
        ...
        CheckBox
        {
            ...
            checked: QD.Preferences.getValue("qidi/choice_on_open_project") != "always_ask"
            style: CheckBoxStyle {
                label: Label {
                    text: control.text
                    font: QD.Theme.getFont("default")
