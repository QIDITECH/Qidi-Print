...
##########Z为0时，底面颜色为蓝
#        if float(Selection.getBoundingBox().bottom) == 0:
            self._enabled_shader.setUniformValue("u_overhangColor", Color(*self._theme.getColor("color5").getRgb()))
#        else:
#            self._enabled_shader.setUniformValue("u_overhangColor", Color(*self._theme.getColor("color17").getRgb()))
            ...
            if not node.render(renderer):
                if node.getMeshData() and node.isVisible() and not node.callDecoration("getLayerData"):
                    ...
                    if stack.getProperty("machine_extruder_count", "value") > 1:
                        if extruder_index == 1:
                            material_color = Preferences.getInstance().getValue("color/extruder2")
                        else:#if extruder_index == 0:
                            material_color = Preferences.getInstance().getValue("color/extruder1")

                    if extruder_index != ExtruderManager.getInstance().activeExtruderIndex:
                        # Shade objects that are printed with the non-active extruder 25% darker
                        shade_factor = 0.45###打印喷头与选择喷头不对应时的颜色深浅
                    ...
                    if node.getName() == "Prime Tower 1" or node.getName() == "Ooze Shield 1" or node.getName() == "Prime Tower 2" or node.getName() == "Ooze Shield 2":
                        ...
                        renderer.queueNode(node, shader = self._ooze_prevention_shader, uniforms = uniforms, transparent = True)
