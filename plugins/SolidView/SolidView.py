...
class SolidView(View):
    ...
    def __init__(self):
        ...
        self._xray_warning_message = Message(
            catalog.i18nc("@info:status", "The highlighted areas indicate either missing or extraneous surfaces. Fix your model and open it again into QIDI."),
    ...
    def _onNonManifoldLearnMoreClicked(self, action, message) -> None:
        QDesktopServices.openUrl(QUrl("https://support.qidi.com/hc/en-us/articles/360014055959"))
    ...
    def _checkSetup(self):
        ...
        if not self._ooze_prevention_shader:
            ...
            self._ooze_prevention_shader.setUniformValue("u_opacity", 0.7)
            self._ooze_prevention_shader.setUniformValue("u_overhangAngle", math.cos(math.radians(45 )))
        ...
        if not Application.getInstance().getPreferences().getValue(self._show_xray_warning_preference):
            ...
            if not self._xray_composite_shader:
                ...
                self._xray_composite_shader.setUniformValue("u_background_color", Color(*theme.getColor("blue_5").getRgb()))
    ...
    def beginRendering(self):
        ...
        for node in DepthFirstIterator(scene.getRoot()):
            ...
            if node.getMeshData() and node.isVisible():
                ...
                #if Application.getInstance().getPreferences().getValue("view/use_extruder_color"):
                #    if Application.getInstance().getPreferences().getValue("color/extruder" + str(extruder_index)):
                #       material_color = str(Application.getInstance().getPreferences().getValue("color/extruder" + str(extruder_index)))
                #    else:
                #        material_color
                ...
                if  node.getName() == "Prime Tower 1" or node.getName() == "Prime Tower 2" or  node.getName() == "Ooze Shield 1"  or node.getName() == "Ooze Shield 2":
                        if getattr(node, "_outside_buildarea", False):
                            ...
                            renderer.queueNode(node, shader = self._ooze_prevention_shader, uniforms = uniforms, test = True)
                elif node.callDecoration("isNonPrintingMesh"):
