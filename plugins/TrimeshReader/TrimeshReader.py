# Copyright (c) 2019 QIDI B.V., fieldOfView
...
from QD.Mesh.MeshData import MeshData, calculateNormalsFromIndexedVertices  # To construct meshes from the Trimesh data.
from QD.Mesh.MeshReader import MeshReader  # The plug-in type we're extending.
from QD.MimeTypeDatabase import MimeTypeDatabase, MimeType  # To add file types that we can open.
from QD.Scene.GroupDecorator import GroupDecorator  # Added to the parent node if we load multiple nodes at once.

from qidi.QIDIApplication import QIDIApplication
from qidi.Scene.BuildPlateDecorator import BuildPlateDecorator  # Added to the resulting scene node.
from qidi.Scene.ConvexHullDecorator import ConvexHullDecorator  # Added to group nodes if we load multiple nodes at once.
from qidi.Scene.QIDISceneNode import QIDISceneNode  # To create a node in the scene after reading the file.
from qidi.Scene.SliceableObjectDecorator import SliceableObjectDecorator  # Added to the resulting scene node.
...
class TrimeshReader(MeshReader):
    ...
    def _toMeshData(self, tri_node: trimesh.base.Trimesh, file_name: str = "") -> MeshData:
        """Converts a Trimesh to QDTECH's MeshData.
        ...
        :return: Mesh data from the Trimesh in a way that QDTECH can understand it.
        """
