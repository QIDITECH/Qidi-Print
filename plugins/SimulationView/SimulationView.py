...
from QD.Event import Event, KeyEvent
...
from QD.View.CompositePass import CompositePass
...
from qidi.QIDIView import QIDIView
from qidi.LayerPolygon import LayerPolygon  # To distinguish line types.
from qidi.Scene.ConvexHullNode import ConvexHullNode
...
class SimulationView(QIDIView):
    ...
    def __init__(self, parent = None) -> None:
        ...
        self._wireprint_warning_message = Message(catalog.i18nc("@info:status", "QIDI does not acqiditely display layers when Wire Printing is enabled."),
        ...
        QIDIApplication.getInstance().getPreferences().addPreference(self._no_layers_warning_preference, True)
    ...
    def beginRendering(self) -> None:
        ...
        if not self._ghost_shader:
            ...
            theme = QIDIApplication.getInstance().getTheme()
        ...
        for node in DepthFirstIterator(scene.getRoot()):
            ...
            if not node.render(renderer):
                if (node.getMeshData()) and node.isVisible():
                    ...
                    # renderer.queueNode(node,shader = self._ghost_shader,
                                       # type = RenderBatch.RenderType.Solid,
                                       # blend_mode = RenderBatch.BlendMode.Transparent,
                                       # sort = -10,
    ...
    def calculateMaxLayers(self) -> None:
        ...
        for node in DepthFirstIterator(scene.getRoot()):  # type: ignore
            ...
            QIDIApplication.getInstance().settest(True)
        ...
        if event.type == Event.ViewActivateEvent:
            ...
            if Platform.isOSX():
                if QOpenGLContext.currentContext() is None:
                    ...
                    QIDIApplication.getInstance().callLater(lambda e=event: self.event(e))
                    return False
            ...
            if not self._simulationview_composite_shader:
                ...
                theme = QIDIApplication.getInstance().getTheme()
                if theme is not None:
                    self._simulationview_composite_shader.setUniformValue("u_background_color", Color(*theme.getColor("blue_5").getRgb()))
    ...
    def _updateSliceWarningVisibility(self):
        if not self.getActivity()\
                and not QIDIApplication.getInstance().getPreferences().getValue("general/auto_slice")\
                and QIDIApplication.getInstance().getPreferences().getValue(self._no_layers_warning_preference):
            self._slice_first_warning_message.show()
        else:
            self._slice_first_warning_message.hide()

    def _onDontAskMeAgain(self, checked: bool) -> None:
        QIDIApplication.getInstance().getPreferences().setValue(self._no_layers_warning_preference, not checked)
