...
from QD.Backend import Backend
...
from qidi.LayerDataBuilder import LayerDataBuilder
from qidi.LayerDataDecorator import LayerDataDecorator
from qidi.LayerPolygon import LayerPolygon
from qidi.Scene.QIDISceneNode import QIDISceneNode
from qidi.Scene.GCodeListDecorator import GCodeListDecorator
...
class FlavorParser:
    ...
    def __init__(self) -> None:
        QIDIApplication.getInstance().hideMessageSignal.connect(self._onHideMessage)
        ...
        QIDIApplication.getInstance().getPreferences().addPreference("gcodereader/show_caution", True)
    ...
    #
    def processGCodeStream(self, stream: str, filename: str) -> Optional["QIDISceneNode"]:
        backend = QIDIApplication.getInstance().getBackend()
        #Logger.log("e","just here")
        ...
        active_build_plate_id = QIDIApplication.getInstance().getMultiBuildPlateModel().activeBuildPlate
        gcode_dict = {active_build_plate_id: gcode_list}
        QIDIApplication.getInstance().getController().getScene().gcode_dict = gcode_dict #type: ignore #Because gcode_dict is generated dynamically.
        ...
        if QIDIApplication.getInstance().getPreferences().getValue("gcodereader/show_caution"):
            caution_message = Message(catalog.i18nc(
                "@info:generic",
                "Make sure the g-code is suitable for your printer and printer configuration before sending the file to it. The g-code representation may not be acqidite."),
                lifetime=0,
                title = catalog.i18nc("@info:title", "G-code Details"))
