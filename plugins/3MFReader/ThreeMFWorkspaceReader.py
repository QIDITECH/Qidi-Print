...
from QD.Settings.ContainerFormatError import ContainerFormatError
from QD.Settings.ContainerStack import ContainerStack
from QD.Settings.DefinitionContainer import DefinitionContainer
...
from qidi.Settings.QIDIContainerStack import _ContainerIndexes
...
class ThreeMFWorkspaceReader(WorkspaceReader):
    ...
    def __init__(self) -> None:
        ...
        # In QIDI 2.5 and 2.6, the empty profiles used to have those long names
    ...
    def preRead(self, file_name, show_dialog=True, *args, **kwargs):
        ...
        qidi_file_names = [name for name in archive.namelist() if name.startswith("Cura/")]
        ...
        definition_container_files = [name for name in qidi_file_names if name.endswith(self._definition_container_suffix)]
        for definition_container_file in definition_container_files:
            container_id = self._stripFileToId(definition_container_file)
            # Logger.log("e",container_id)
            #container_id = ""
            ...
            if definition_container_type == "machine":
                ...
                self.machine_name_for_icon =machine_type
        ...
        if xml_material_profile:
            material_container_files = [name for name in qidi_file_names if name.endswith(self._material_container_suffix)]
        ...
        instance_container_files = [name for name in qidi_file_names if name.endswith(self._instance_container_suffix)]
        quality_name = ""
        custom_quality_name = ""
        intent_name = ""
        intent_category = ""
        ...
        #Logger.log("e",serialized)
        #Logger.log("e", global_stack_file)
        ...
        #Logger.log("e",len(id_list))
        if id_list[-1] != machine_definition_id:
            machine_definition_id = id_list[-1]
        ...
        parser.read_string(serialized)
        if len(id_list) == 7:
            quality_container_id = parser["containers"][str(_ContainerIndexes.Quality-1)]
        else:
            quality_container_id = parser["containers"][str(_ContainerIndexes.Quality)]
        #Logger.log("e",str(_ContainerIndexes.Quality))
        ...
        parser.read_string(serialized)
        if len(id_list) == 7:
            definition_changes_id = parser["containers"][str(_ContainerIndexes.DefinitionChanges-1)]
        else:
        ...
        if not extruder_stack_files:
            ...
            if len(id_list) == 7:
                variant_id = parser["containers"][str(_ContainerIndexes.Variant-1)]
            else:
                variant_id = parser["containers"][str(_ContainerIndexes.Variant)]
            if len(id_list) == 7:
                material_id = parser["containers"][str(_ContainerIndexes.Material-1)]
            else:
        ...
        for extruder_stack_file in extruder_stack_files:
            ...
            id_list2 = self._getContainerIdListFromSerialized(serialized)
            #Logger.log("e",serialized)
            if len(id_list2) == 7:
                variant_id = parser["containers"][str(_ContainerIndexes.Variant-1)]
            else:
            ...
            definition_changes_id = parser["containers"][str(_ContainerIndexes.DefinitionChanges)]
            #Logger.log("e",parser["containers"][str(_ContainerIndexes.UserChanges)])
            if len(id_list2) == 7:
                definition_changes_id = parser["containers"][str(_ContainerIndexes.DefinitionChanges-1)]
            else:
                definition_changes_id = parser["containers"][str(_ContainerIndexes.DefinitionChanges)]
        ...
        try:
            ...
            active_mode = temp_preferences.getValue("qidi/active_mode")
            if not active_mode:
                active_mode = Application.getInstance().getPreferences().getValue("qidi/active_mode")
        except KeyError:
    ...
    def read(self, file_name):
        ...
        if visible_settings is None:
            ...
            global_preferences.setValue("qidi/active_setting_visibility_preset", "custom")

        categories_expanded = temp_preferences.getValue("qidi/categories_expanded")
        if categories_expanded is None:
            Logger.log("w", "Workspace did not contain expanded categories. Leaving them unchanged")
        else:
            global_preferences.setValue("qidi/categories_expanded", categories_expanded)

        application.expandedCategoriesChanged.emit()  # Notify the GUI of the change
        #Logger.log("e",self._machine_info.name)
        ...
        if self._resolve_strategies["machine"] != "override" or self._dialog.updatableMachinesModel.count <= 1:
            ...
            machine_name = self._machine_info.name
            ...
            definitions = self._container_registry.findDefinitionContainers(id=self._machine_info.definition_id)
            global_stack = QIDIStackBuilder.createMachine(definitions[0].name, self._machine_info.definition_id, machine_extruder_count)
            ...
            if not global_stacks:
                message = Message(i18n_catalog.i18nc("@info:error Don't translate the XML tag <filename>!", 
                                                     ...
                                                     " are unknown to this version of QIDI TECH.", file_name))
                message.show()
                self.setWorkspaceName("")
                return [], {}
        ...
        if xml_material_profile:
            ...
            for material_container_file in material_container_files:
                ...
                if to_deserialize_material:
                    ...
                    # Logger.log("e",material_container)
                    # Logger.log("e",archive.open(material_container_file).read().decode("utf-8"))
                    try:
                    ...
                    new_metadata = {"brand": i18n_catalog.i18nc("@label", "Custom")}
                    material_container.getMetaData().update(new_metadata)
        ...
        if len(WifiSend.getInstance().FullNameIPList) >0 :
            WifiSend.getInstance().setCurrentDeviceIP(WifiSend.getInstance().FullNameIPList[0])
    ...
    def _processQualityChanges(self, global_stack):
        ...
        if self._machine_info.quality_changes_info is not None:
            ...
            if not quality_changes_info.extruder_info_dict:
                ...
                # If the global stack we're "targeting" has never been active, but was updated from QIDI 3.4,
    ...
    def _createNewQualityChanges(self, quality_type: str, intent_category: Optional[str], name: str, global_stack: GlobalStack, extruder_stack: Optional[ExtruderStack]) -> InstanceContainer:
        ...
        container_registry = QIDIApplication.getInstance().getContainerRegistry()
        ...
        quality_changes.setMetaDataEntry("setting_version", QIDIApplication.getInstance().SettingVersion)
            ...
            if extruder_info.user_changes_info is not None:
                ...
                if parser is not None:
                    for key, value in parser["values"].items():
                        ...
                        #Logger.log("e",key)
                        #Logger.log("e",value)
                        #Logger.log("e",self.machine_name_for_icon)
                        #self._icon_change.append(key+"*"+position)
                        #application.getInstance().parameter_list(self.machine_name_for_icon,key)
                        extruder_stack.userChanges.setProperty(key, "value", value)
       # application.getInstance().parameter_list(self.machine_name_for_icon,self._icon_change)
    ...
    def _applyMaterials(self, global_stack, extruder_stack_dict):
        ...
        for position, extruder_stack in extruder_stack_dict.items():
            ...
            try:
                ...
                message = Message(
                                    i18n_catalog.i18nc("@info:status", "Your configuration seems to be corrupt,The materials were changed to pla."),
                                    lifetime = 0,
                                    title = i18n_catalog.i18nc("@info:title", "Configuration errors")
                                )
                message.show()
                material_node = machine_node.variants[extruder_stack.variant.getName()].materials['generic_pla']
                extruder_stack.material = material_node.container  # type: InstanceContainer
    ...
    def _getXmlProfileClass(self):
        return self._container_registry.getContainerForMimeType(MimeTypeDatabase.getMimeType("application/x-qidi-material-profile"))
    ...
    def _getMaterialLabelFromSerialized(serialized):
        ...
        metadata = data.iterfind("./qd:metadata/qd:name/qd:label", {"qd": "http://www.qidi.com/material"})
