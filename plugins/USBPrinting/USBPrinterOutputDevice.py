...
from QD.Mesh.MeshWriter import MeshWriter #To get the g-code output.
from QD.Message import Message #Show an error when already printing.
...
from qidi.PrinterOutput.PrinterOutputDevice import PrinterOutputDevice, ConnectionState, ConnectionType
...
from qidi.PrinterOutput.GenericOutputController import GenericOutputController
...
class USBPrinterOutputDevice(PrinterOutputDevice):
    def __init__(self, serial_port: str, baud_rate: Optional[int] = None) -> None:
        ...
        QIDIApplication.getInstance().getOnExitCallbackManager().addCallback(self._checkActivePrintingUponAppExit)
    ...
    def _checkActivePrintingUponAppExit(self) -> None:
        ...
        application.showConfirmExitDialog.emit(catalog.i18nc("@label", "A USB print is in progress, closing QIDI will stop this print. Are you sure?"))
    ...
    def requestWrite(self, nodes: List["SceneNode"], file_name: Optional[str] = None, limit_mimetypes: bool = False,
        ...
        if self._is_printing:
            message = Message(text = catalog.i18nc("@message", "A print is still in progress. QIDI cannot start another print via USB until the previous print has completed."), title = catalog.i18nc("@message", "Print in Progress"))
            message.show()
        ...
        QIDIApplication.getInstance().getController().setActiveStage("MonitorStage")
    ...
    def _printGCode(self, gcode: str):
        ...
        self._print_estimated_time = int(QIDIApplication.getInstance().getPrintInformation().currentPrintTime.getDisplayString(DurationFormat.Format.Seconds))
    ...
    def connect(self):
        ...
        if self._serial is None:
            try:
                ...
                return
        QIDIApplication.getInstance().globalContainerStackChanged.connect(self._onGlobalContainerStackChanged)
        self._onGlobalContainerStackChanged()
        self.setConnectionState(ConnectionState.Connected)
        self._update_thread.start()

    def _onGlobalContainerStackChanged(self):
        container_stack = QIDIApplication.getInstance().getGlobalContainerStack()
    ...
    def _update(self):
        while self._connection_state == ConnectionState.Connected and self._serial is not None:
            ...
            if line == b"":
                ...
                # Multiple empty lines probably means that the firmware and QIDI are waiting
    ...
    def _sendNextGcodeLine(self):
        ...
        if print_job is None:
            controller = GenericOutputController(self)
            controller.setCanUpdateFirmware(True)
            print_job = PrintJobOutputModel(output_controller = controller, name = QIDIApplication.getInstance().getPrintInformation().jobName)
