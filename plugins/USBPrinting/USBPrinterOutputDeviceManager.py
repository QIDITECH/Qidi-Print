...
from qidi.PrinterOutput.PrinterOutputDevice import ConnectionState
...
class USBPrinterOutputDeviceManager(QObject, OutputDevicePlugin):
    ...
    def getSerialPortList(self, only_list_usb = False):
        ...
        for port in port_list:
            ...
            # To prevent qidi from messing with serial ports of other devices,
            ...
            # set QIDI_DEVICENAMES=USB[1-9] -> e.g. not matching /dev/ttyUSB0
            pattern = environ.get('QIDI_DEVICENAMES')
            if pattern and not search(pattern, port[0]):
                continue

            # set QIDI_DEVICETYPES=CP2102 -> match a type of serial converter
            pattern = environ.get('QIDI_DEVICETYPES')
            if pattern and not search(pattern, port[1]):
                continue

            # set QIDI_DEVICEINFOS=LOCATION=2-1.4 -> match a physical port
            # set QIDI_DEVICEINFOS=VID:PID=10C4:EA60 -> match a vendor:product
            pattern = environ.get('QIDI_DEVICEINFOS')
