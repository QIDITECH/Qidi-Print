...
class X3DReader(MeshReader):
    ...
    def _read(self, file_name):
        try:
            ...
            scale = 1000 # Default X3D unit it one meter, while QIDI's is one millimeters
    ...
    def processTransform(self, node):
        ...
        # Store the previous transform; in QIDI, the default matrix multiplication is in place
        ...
        if scale.x != 1 or scale.y != 1 or scale.z != 1:
            ...
            # No scale by vector in place operation in QD
            S = Matrix()
...
# Triangle meshes
    ...
    # Won't work for now, since QIDI expects every mesh to have a nontrivial convex hull
    ...
    def reserveFaceAndVertexCount(self, num_faces, num_verts):
        # Unlike the QIDI MeshBuilder, we use 4-vectors stored as columns for easier transform
