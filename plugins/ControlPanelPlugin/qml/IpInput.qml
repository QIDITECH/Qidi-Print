...
Rectangle {
    ...
    height: 40
    color: "white"
    border.color: QD.Theme.getColor("gray_1")//"black"
    property int _itemWidth: (width-3*style.width)/4
	property string iptext: style.text
    Text{
        id: style
        visible: false
        text: "·"
        ...
        color: QD.Theme.getColor("black_1")//"white"
    }
    ...
    Row {
        ...
        Repeater{
            ...
            IpItem {
				...
				onTextChanged: {
					if (repeater.itemAt(0).text)
					{
						iptext=repeater.itemAt(0).text  + "." + repeater.itemAt(2).text + "."+ repeater.itemAt(4).text + "." + repeater.itemAt(6).text
					}
					else
					{
						iptext = "0.0.0.0"
