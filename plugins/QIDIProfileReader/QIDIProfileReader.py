...
from qidi.ReaderWriters.ProfileReader import ProfileReader

import zipfile


class QIDIProfileReader(ProfileReader):
    """A plugin that reads profile data from QIDI profile files.

    It reads a profile from a .qidiprofile file, and returns it as a profile
    instance.
    """

    def __init__(self) -> None:
        """Initialises the qidi profile reader.
    ...
    def read(self, file_name: str) -> List[Optional[InstanceContainer]]:
        """Reads a qidi profile from a file and returns it.

        :param file_name: The file to read the qidi profile from.
        :return: The qidi profiles that were in the file, if any. If the file
        ...
        try:
            ...
            # It must be an older profile from QIDI 2.1.
    ...
    def _upgradeProfile(self, serialized: str, profile_id: str) -> List[Tuple[str, str]]:
        """Convert a profile from an old QIDI to this QIDI if needed.
    ...
    def _upgradeProfileVersion(self, serialized: str, profile_id: str, main_version: int, setting_version: int) -> List[Tuple[str, str]]:
        ...
        if int(new_source_version / 1000000) != InstanceContainer.Version or new_source_version % 1000000 != QIDIApplication.SettingVersion:
