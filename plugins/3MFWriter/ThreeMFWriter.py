...
class ThreeMFWriter(MeshWriter):
    def __init__(self):
        ...
        self._namespaces = {
            ...
            "qidi": "http://software.qidi.com/xml/qidi/3mf/2015/10"
        }
    ...
    def _convertQDNodeToSavitarNode(self, um_node, transformation = Matrix()):
        """Convenience function that converts an QDTECH SceneNode object to a SavitarSceneNode

        :returns: QDTECH Scene node.
        """
        if not isinstance(um_node, SceneNode):
            return None

        active_build_plate_nr = QIDIApplication.getInstance().getMultiBuildPlateModel().activeBuildPlate
        ...
        for child_node in um_node.getChildren():
            ...
            if child_node.callDecoration("getBuildPlateNumber") != active_build_plate_nr:
                continue
            savitar_child_node = self._convertQDNodeToSavitarNode(child_node)
    ...
    def write(self, stream, nodes, mode = MeshWriter.OutputMode.BinaryMode):
        ...
        try:
            ...
            metadata_to_store = QIDIApplication.getInstance().getController().getScene().getMetaData()
            ...
            if "Application" not in metadata_to_store:
                ...
                savitar_scene.setMetaDataEntry("Application", QIDIApplication.getInstance().getApplicationDisplayName())
            ...
            # Second step: 3MF defines the left corner of the machine as center, whereas qidi uses the center of the
            # build volume.
            ...
            root_node = QD.Application.Application.getInstance().getController().getScene().getRoot()
            for node in nodes:
                if node == root_node:
                    for root_child in node.getChildren():
                        savitar_node = self._convertQDNodeToSavitarNode(root_child, transformation_matrix)
                        if savitar_node:
                            savitar_scene.addSceneNode(savitar_node)
                else:
                    savitar_node = self._convertQDNodeToSavitarNode(node, transformation_matrix)
                    if savitar_node:
                        savitar_scene.addSceneNode(savitar_node)
