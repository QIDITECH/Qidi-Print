...
Item
{
    ...
    Flow {
        ...
        anchors.left: inchTommButton.right;
    ...
    Button
    {
        ...
        style: UM.Theme.styles.wifi_button//UM.Theme.styles.translate_button

        onClicked:
        {
            if (!UM.ActiveTool.properties.getValue("NonUniformScale"))
            {
                UM.ActiveTool.setProperty("ScaleX", parseFloat(xPercentage.text) / 100 * 25.4)
            }
            else
            {
                UM.ActiveTool.setProperty("ScaleX", parseFloat(xPercentage.text) / 100 * 25.4)
                UM.ActiveTool.setProperty("ScaleZ", parseFloat(zPercentage.text) / 100 * 25.4)
                UM.ActiveTool.setProperty("ScaleY", parseFloat(yPercentage.text) / 100 * 25.4)
            }
        }
        z: 1
    }
