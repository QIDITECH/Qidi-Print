...
from QD.Event import MouseEvent
...
class SelectionTool(Tool):
    ...
    def event(self, event):
        ...
        if self._button_pass is None:
            self._button_pass = self._renderer.getRenderPass("button")
        self.checkModifierKeys(event)
        if event.type == MouseEvent.MousePressEvent :
            id=self._button_pass.getIdAtPosition(event.x, event.y)
