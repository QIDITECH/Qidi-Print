...
# QIDI is released under the terms of the AGPLv3 or higher.
# Initial source code qidi-god-mode-plugin from sedwards2009
# https://github.com/sedwards2009/qidi-god-mode-plugin
#
...
#
# V 1.2.0  01/05/2022 Update for QIDI 5.0
...
i18n_qidi_catalog = i18nCatalog('qidi')
...
def viewAll():
    HtmlFile = str(QIDIVersion).replace('.','-') + '_qidi_settings.html'
    openHtmlPage(HtmlFile, htmlPage())

def viewCompare():
    HtmlFile = str(QIDIVersion).replace('.','-') + '_qidi_settings_compare.html'
    openHtmlPage(HtmlFile, htmlComparePage())

def viewAllQualityChanges():
    HtmlFile = str(QIDIVersion).replace('.','-') + '_qidi_quality_changes.html'
...
def htmlComparePage():
    ...
    extruder_count=int(QIDIApplication.getInstance().getGlobalContainerStack().getProperty('machine_extruder_count', 'value'))
    ...
    for prop_name in properties:
        ...
        if prop_value is not None:
            if prop_name=='value' :
                ...
                if print_value.find('QD.Settings.SettingFunction') > 0 :
...
def formatSettingsKeyTableRow(key, value):
    ...
    Cstack = QIDIApplication.getInstance().getGlobalContainerStack()
...
def openHtmlPage(page_name, html_contents):
    ...
    if not has_browser() :
        ...
        Message(text = "Default browser not defined open \n %s" % (target), title = i18n_qidi_catalog.i18nc("@info:title", "Warning ! ProfilAnalyser")).show()
        
    QDesktopServices.openUrl(QUrl.fromLocalFile(target))

def getHtmlHeader(page_name='QIDI Settings'):
