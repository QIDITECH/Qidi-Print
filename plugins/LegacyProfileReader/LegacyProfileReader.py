...
from QD.Application import Application  # To get the machine manager to create the new profile in.
from QD.Logger import Logger  # Logging errors.
from QD.PluginRegistry import PluginRegistry  # For getting the path to this plugin's directory.
from QD.Settings.ContainerRegistry import ContainerRegistry #To create unique profile IDs.
from QD.Settings.InstanceContainer import InstanceContainer  # The new profile to make.
from qidi.ReaderWriters.ProfileReader import ProfileReader  # The plug-in type to implement.


class LegacyProfileReader(ProfileReader):
    """A plugin that reads profile data from legacy QIDI versions.
    ...
    def prepareDefaults(self, json: Dict[str, Dict[str, str]]) -> Dict[str, str]:
        ...
        :return: A dictionary of the default values of the legacy QIDI version.
        """
    ...
    def prepareLocals(self, config_parser, config_section, defaults):
        ...
        This fills a dictionary with all settings from the legacy QIDI version
        ...
        :param config_section: The section in the profile where the settings
            should be found.
        :param defaults: The default values for all settings in the legacy QIDI.
    ...
    def read(self, file_name):
        """Reads a legacy QIDI profile from a file and returns it.

        :param file_name: The file to read the legacy QIDI profile from.
        :return: The legacy QIDI profile that was in the file, if any. If the
        ...
        # Legacy QIDI saved the profile under the section "profile_N" where N is the ID of a machine, except when you export in which case it saves it in the section "profile".
        ...
        for new_setting in dict_of_doom["translation"]:  # Evaluate all new settings that would get a value from the translations.
            ...
            if definitions:
                if new_value != value_using_defaults and definitions[0].default_value != new_value:  # Not equal to the default in the new QIDI OR the default in the legacy QIDI.
