...
import QD.Application # To mock the application out.
import QD.PluginRegistry # To mock the plug-in registry out.
import QD.Settings.ContainerRegistry # To mock the container registry out.
import QD.Settings.InstanceContainer # To intercept the serialised data from the read() function.
...
def test_read(legacy_profile_reader, file_name):
    ...
    QD.Settings.InstanceContainer.setContainerRegistry(container_registry)
    ...
    with unittest.mock.patch.object(QD.Application.Application, "getInstance", application_getInstance):
        with unittest.mock.patch.object(QD.Settings.ContainerRegistry.ContainerRegistry, "getInstance", container_registry_getInstance):
            with unittest.mock.patch.object(QD.PluginRegistry.PluginRegistry, "getInstance", plugin_registry_getInstance):
                with unittest.mock.patch.object(QD.Settings.InstanceContainer.InstanceContainer, "deserialize", deserialize):
                    with unittest.mock.patch.object(QD.Settings.InstanceContainer.InstanceContainer, "duplicate", duplicate):
