...
from QD.Backend.Backend import Backend, BackendState
...
from QD.Settings.Interfaces import DefinitionContainerInterface
from QD.Settings.SettingInstance import SettingInstance #For typing.
from QD.Tool import Tool #For typing.
...
from .OozePrevention import OozePrevention
...
if TYPE_CHECKING:
    ...
    from QD.Scene.Scene import Scene
...
class CuraEngineBackend(QObject, Backend):
    ...
    def __init__(self) -> None:
        ...
        QIDIEngineBackend is exposed to qml as well.
        """

        super().__init__()
        # Find out where the engine is located, and how it is called.
        # This depends on how QIDI is packaged and which OS we are running on.
        ...
        search_path = [
            ...
            os.path.join(QIDIApplication.getInstallPrefix(), "bin"),
        ...
        application = QIDIApplication.getInstance() #type: QIDIApplication
        ...
        Logger.log("i", "Found QIDIEngine at: %s", self._default_engine_location)
        ...
        # self._slicing_error_message.addAction(
            ...
            # description = catalog.i18nc("@message:description", "Report a bug on QIDI's issue tracker."),
            # icon = "[no_icon]"
        # )
    ...
    def close(self) -> None:
        ...
        # Terminate QIDIEngine if it is still running at this point
        self._terminate()
    ...
    def getEngineCommand(self) -> List[str]:
        ...
        if ApplicationMetadata.IsEnterpriseVersion:
            ...
            command = [QIDIApplication.getInstance().getPreferences().getValue("backend/location")]
    ...
    def _createSnapshot(self) -> None:
        self._snapshot = None
        if not QIDIApplication.getInstance().isVisible:
    ...
    def _terminate(self) -> None:
        ...
        if QIDIApplication.getInstance().getUseExternalBackend():
            return
    ...
    def determineAutoSlicing(self) -> bool:
        ...
        if not QIDIApplication.getInstance().getPreferences().getValue("general/auto_slice"):
            enable_timer = False
    ...
    def _onSocketError(self, error: Arcus.Error) -> None:
        ...
        if QIDIApplication.getInstance().isShuttingDown():
            return
    ...
    def markSliceAll(self) -> None:
        for build_plate_number in range(QIDIApplication.getInstance().getMultiBuildPlateModel().maxBuildPlate + 1):
            if build_plate_number not in self._build_plates_to_be_sliced:
                self._build_plates_to_be_sliced.append(build_plate_number)
    ...
    def _createSocket(self, protocol_file: str = None) -> None:
        ...
        if not protocol_file:
            if not self.getPluginId():
                Logger.error("Can't create socket before QIDIEngineBackend plug-in is registered.")
                return
            plugin_path = PluginRegistry.getInstance().getPluginPath(self.getPluginId())
            if not plugin_path:
                Logger.error("Could not get plugin path!", self.getPluginId())
                return
            protocol_file = os.path.abspath(os.path.join(plugin_path, "QIDI.proto"))
    ...
    def _onActiveViewChanged(self) -> None:
        ...
        view = QIDIApplication.getInstance().getController().getActiveView()
        if view:
    ...
    def _reportBackendError(self, _message_id: str, _action_id: str) -> None:
        ...
        """
        QDesktopServices.openUrl(QUrl("https://github.com/Ultimaker/QIDI/issues/new/choose"))
    ...
    def _onGlobalStackChanged(self) -> None:
        ...
        self._global_container_stack = QIDIApplication.getInstance().getMachineManager().activeMachine
