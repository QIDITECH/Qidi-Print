...
class OozePrevention(QObject):
    ...
    def addOozeWall(self):
        ...
        wall_thickness = (stack0.getProperty("wall_line_width", "value") + stack1.getProperty("wall_line_width", "value")) * self._stack.getProperty("ooze_wall_line_count", "value")
    ...
    def addWall(self, parent: QIDISceneNode, outer_points, inner_points, height = 10):
        node = QIDISceneNode()
                ...
                while True:
                    ...
                    for node in DepthFirstIterator(self._scene.getRoot()):
                        if node.callDecoration("isSliceable"):
                            ...
                            if node.collidesWithAreas([target_area]):
    ...
    def addTower(self, parent: QIDISceneNode, position: Vector, size = 20, height = 10):
        node = QIDISceneNode()
        node.setSelectable(True)
        mesh = MeshBuilder()
        mesh.createCube(
        ...
        op.addOperation(SetParentOperation(node, parent))
        op.push()
        node.setPosition(position, QIDISceneNode.TransformSpace.World)

        self._scene.sceneChanged.emit(node)
        return node

    def settingDefinition(self, node: QIDISceneNode, definitionName: str, value):
    ...
    def setExtruder(self, node: QIDISceneNode, position: 0):
