...
from QD.Settings.ContainerStack import ContainerStack #For typing.
from QD.Settings.InstanceContainer import InstanceContainer
from QD.Settings.Interfaces import ContainerInterface
from QD.Settings.SettingDefinition import SettingDefinition
from QD.Settings.SettingRelation import SettingRelation #For typing.

from QD.Scene.Iterator.DepthFirstIterator import DepthFirstIterator
from QD.Scene.Scene import Scene #For typing.
from QD.Settings.Validator import ValidatorState
from QD.Settings.SettingRelation import RelationType

from qidi.QIDIApplication import QIDIApplication
from qidi.Scene.QIDISceneNode import QIDISceneNode
from qidi.OneAtATimeIterator import OneAtATimeIterator
...
class StartSliceJob(Job):
    ...
    def __init__(self, slice_message: Arcus.PythonMessage) -> None:
        ...
        self._scene = QIDIApplication.getInstance().getController().getScene() #type: Scene
    ...
    def run(self) -> None:
        ...
        stack = QIDIApplication.getInstance().getGlobalContainerStack()
        if not stack:
            self.setResult(StartJobResult.Error)
            return

        # Don't slice if there is a setting with an error value.
        if QIDIApplication.getInstance().getMachineManager().stacksHaveErrors:
            self.setResult(StartJobResult.SettingError)
            return

        if QIDIApplication.getInstance().getBuildVolume().hasErrors():
            self.setResult(StartJobResult.BuildPlateError)
            return

        # Wait for error checker to be done.
        while QIDIApplication.getInstance().getMachineErrorChecker().needToWaitForResult:
            time.sleep(0.1)

        if QIDIApplication.getInstance().getMachineErrorChecker().hasError:
            self.setResult(StartJobResult.SettingError)
            return

        # Don't slice if the buildplate or the nozzle type is incompatible with the materials
        if not QIDIApplication.getInstance().getMachineManager().variantBuildplateCompatible and \
                not QIDIApplication.getInstance().getMachineManager().variantBuildplateUsable:
        ...
        for node in DepthFirstIterator(self._scene.getRoot()):
            if not isinstance(node, QIDISceneNode) or not node.isSelectable():
                continue
        ...
        global_stack = QIDIApplication.getInstance().getGlobalContainerStack()
        if not global_stack:
            return
        ...
        for group in filtered_object_groups:
            ...
            if parent is not None and parent.callDecoration("isGroup"):
                self._handlePerObjectSettings(cast(QIDISceneNode, parent), group_message)
            ...
            for object in group:
                ...
                obj.id = id(object)
                obj.name = object.getName().replace(" ","_")
                ...
                self._handlePerObjectSettings(cast(QIDISceneNode, object), obj)
    ...
    def _buildReplacementTokens(self, stack: ContainerStack) -> Dict[str, Any]:
        ...
        result["initial_extruder_nr"] = QIDIApplication.getInstance().getExtruderManager().getInitialExtruderNr()

        return result

    def _cacheAllExtruderSettings(self):
        global_stack = cast(ContainerStack, QIDIApplication.getInstance().getGlobalContainerStack())
    ...
    def _buildGlobalSettingsMessage(self, stack: ContainerStack) -> None:
        ...
        initial_extruder_nr = QIDIApplication.getInstance().getExtruderManager().getInitialExtruderNr()
    ...
    def _handlePerObjectSettings(self, node: QIDISceneNode, message: Arcus.PythonMessage):
