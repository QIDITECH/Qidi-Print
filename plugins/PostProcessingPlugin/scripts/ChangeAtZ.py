...
# This script is the successor of the TweakAtZ plugin for legacy QIDI.
# It contains code from the TweakAtZ plugin V1.0-V4.x and from the ExampleScript by Jaime van Kessel, QIDI B.V.
...
# V4.0.2:	Uses QIDI progress bar instead of its own
# V4.0.3:	Bugfix for cool head lift (contributed by luisonoff)
# V4.9.91:	First version for QIDI 15.06.x and PostProcessingPlugin
# V4.9.92:	Modifications for QIDI 15.10
# V4.9.93:	Minor bugfixes (input settings) / documentation
# V4.9.94:	Bugfix Combobox-selection; remove logger
# V5.0:		Bugfix for fall back after one layer and doubled G0 commands when using print speed tweak, Initial version for QIDI 2.x
# V5.0.1:	Bugfix for calling unknown property 'bedTemp' of previous settings storage and unkown variable 'speed'
# V5.1:		API Changes included for use with QIDI 2.2
...
# Broke up functions for clarity. Split up class so it can be debugged outside of QIDI.
...
# Uses -
# M220 S<factor in percent> - set speed factor override percentage 设置速度因子覆盖百分比
# M221 S<factor in percent> - set flow factor override percentage 设置流量因数覆盖百分比
# M221 S<factor in percent> T<0-#toolheads> - set flow factor override percentage for single extruder 设置单台挤出机的流量系数超调百分比
# M104 S<temp> T<0-#toolheads> - set extruder <T> to target temperature <S> 将挤出机<t>设置为目标温度<s></s></t>
# M140 S<temp> - set bed target temperature 设定床层目标温度
# M106 S<PWM> - set fan speed to target speed <S> 将风扇转速设置为目标转速
# M207 S<mm> F<mm/m> - set the retract length <S> or feed rate <F> 设置缩回长度
# M117 - output the current changes 输出当前的变化
...
# this was broken up into a separate class so the main ChangeZ script could be debugged outside of QIDI
...
class GCodeCommand:
    ...
    # Gets a GCode Command from the given single line of GC-ode
    @staticmethod
...
class ChangeAtZProcessor:
    ...
    def processRetractLength(self, extrude_length: float, feed_rate: float, new_line: str, x_coord: float, y_coord: float, z_coord: float) -> str:
        ...
        # retractions are only F (feed rate) and E (extrude), at least in qidi
    ...
    # Used for picking out the retract length set by QIDI
    def processRetractLengthSetting(self, line: str):
        ...
        # qidi stores the retract length as a negative E just before it starts printing
