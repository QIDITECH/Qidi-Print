# Copyright (c) 2018 Jaime van Kessel, QIDI B.V.
...
from QD.Trust import Trust, TrustBasics
...
class PostProcessingPlugin(QObject, Extension):
    ...
    def __init__(self, parent = None) -> None:
        ...
        QIDIApplication.getInstance().mainWindowChanged.connect(self._createView)  # When the main window is created, create the view so that we can display the post-processing icon if necessary.
    ...
    def loadScripts(self, path: str) -> None:
        ...
        if ApplicationMetadata.IsEnterpriseVersion:
            ...
            install_prefix = os.path.abspath(QIDIApplication.getInstance().getInstallPrefix())
            try:
    ...
    def _createView(self) -> None:
        ...
        self._view = QIDIApplication.getInstance().createQmlComponent(path, {"manager": self})
        ...
        QIDIApplication.getInstance().addAdditionalComponent("saveButton", self._view.findChild(QObject, "postProcessingSaveAreaButton"))
