...
if TYPE_CHECKING:
    from QD.TaskManagement.HttpRequestData import HttpRequestData
    from qidi.Settings.GlobalStack import GlobalStack

i18n_catalog = i18nCatalog("qidi")

DEFAULT_MARKETPLACE_ROOT = "https://marketplace.qidi.com"  # type: str

try:
    from qidi.QIDIVersion import QIDIMarketplaceRoot
except ImportError:
    QIDIMarketplaceRoot = DEFAULT_MARKETPLACE_ROOT
...
class Toolbox(QObject, Extension):
    ...
    def __init__(self, application: QIDIApplication) -> None:
        ...
        self._cloud_scope = QIDICloudScope(application)  
    ...
    def _restart(self):
        ...
        if not self._application.getQIDIAPI().account.isLoggedIn and ApplicationMetadata.IsEnterpriseVersion:
    ...
    def _onAppInitialized(self) -> None:
        ...
        self._application.getQIDIAPI().account.loginStateChanged.connect(self._restart)

        # On boot we check which packages have updates.
        if QIDIApplication.getInstance().getPreferences().getValue("info/automatic_update_check") and len(installed_package_ids_with_versions) > 0:
    ...
    def _onDownloadFinished(self, reply: "QNetworkReply") -> None:
        ...
        self._temp_plugin_file = tempfile.NamedTemporaryFile(mode = "w+b", suffix = ".qidipackage", delete = False)
    ...
    def _onDownloadComplete(self, file_path: str) -> None:
        ...
        if not package_info:
            Logger.log("w", "Package file [%s] was not a valid QIDIPackage.", file_path)
            return
    ...
    def getWebMarketplaceUrl(self, page: str) -> str:
        root = QIDIMarketplaceRoot
        if root == "":
            root = DEFAULT_MARKETPLACE_ROOT
        return root + "/app/qidi/" + page
