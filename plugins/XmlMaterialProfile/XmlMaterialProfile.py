...
from qidi.Machines.VariantType import VariantType
...
class XmlMaterialProfile(InstanceContainer):
    ...
    def xmlVersionToSettingVersion(xml_version: str) -> int:
        ...
        if xml_version == "1.3":
            return QIDIApplication.SettingVersion
    ...
    def setMetaDataEntry(self, key, value, apply_to_all = True):
        ...
        if not apply_to_all:  # Historical: If you only want to modify THIS container. We only used that to prevent recursion but with the below code that's no longer necessary.
            # QIDI-6920: This is an optimization, but it also fixes the problem that you can only set metadata for a
    ...
    def serialize(self, ignored_metadata_keys: Optional[Set[str]] = None):
        ...
        root = builder.start("fdmmaterial",
                             {"xmlns": "http://www.qidi.com/material",
                              "xmlns:qidi": "http://www.qidi.com/qidi",
        ...
        for key, value in metadata.items():
            key_to_use = key
            if key in self._metadata_tags_that_have_qidi_namespace:
                key_to_use = "qidi:" + key_to_use
    ...
    def _resolveInheritance(self, file_name):
        ...
        inherits = xml.find("./qd:inherits", self.__namespaces)
    ...
    def _loadFile(self, file_name):
        path = Resources.getPath(QIDIApplication.getInstance().ResourceTypes.MaterialInstanceContainer, file_name + ".xml.fdm_material")
    ...
    def _expandMachinesXML(self, element):
        settings_element = element.find("./qd:settings", self.__namespaces)
        machines = settings_element.iterfind("./qd:machine", self.__namespaces)
        machines_to_add = []
        machines_to_remove = []
        for machine in machines:
            identifiers = list(machine.iterfind("./qd:machine_identifier", self.__namespaces))
            ...
            if has_multiple_identifiers:
                ...
                for identifier in identifiers:
                    ...
                    new_machine_identifiers = list(new_machine.iterfind("./qd:machine_identifier", self.__namespaces))
    ...
    def deserialize(self, serialized, file_name = None):
        ...
        inherits = data.find("./qd:inherits", self.__namespaces)
        if inherits is not None:
            inherited = self._resolveInheritance(inherits.text)
        ...
        for entry in data.iterfind("./qd:metadata/*", self.__namespaces):
            tag_name = _tag_without_namespace(entry)

            if tag_name == "name":
                brand = entry.find("./qd:brand", self.__namespaces)
                material = entry.find("./qd:material", self.__namespaces)
                color = entry.find("./qd:color", self.__namespaces)
                label = entry.find("./qd:label", self.__namespaces)
        ...
        property_values = {}
        properties = data.iterfind("./qd:properties/*", self.__namespaces)
        ...
        settings = data.iterfind("./qd:settings/qd:setting", self.__namespaces)
        for entry in settings:
            ...
            if key in self.__material_settings_setting_map:
                if key == "processing temperature graph": #This setting has no setting text but subtags.
                    graph_nodes = entry.iterfind("./qd:point", self.__namespaces)
                    graph_points = []
        ...
        # Add namespaced QIDI-specific settings
        settings = data.iterfind("./qd:settings/qidi:setting", self.__namespaces)
        for entry in settings:
            value = entry.text
            if value.lower() == "yes":
                value = True
        ...
        machines = data.iterfind("./qd:settings/qd:machine", self.__namespaces)
        for machine in machines:
            machine_compatibility = common_compatibility
            machine_setting_values = {}
            settings = machine.iterfind("./qd:setting", self.__namespaces)
            for entry in settings:
                key = entry.get("key")
                if key in self.__material_settings_setting_map:
                    if key == "processing temperature graph": #This setting has no setting text but subtags.
            ...
            # Add namespaced QIDI-specific settings
            settings = machine.iterfind("./qidi:setting", self.__namespaces)
            for entry in settings:
                value = entry.text
                if value.lower() == "yes":
                    value = True
            ...
            identifiers = machine.iterfind("./qd:machine_identifier", self.__namespaces)
            for identifier in identifiers:
                ...
                for machine_id in machine_id_list:
                    ...
                    # show as incompatible if the material profile doesn't define hotends in the machine - QIDI-5444
                    ...
                    # project file and the a material in QIDI have the same ID.
                    ...
                    hotends = machine.iterfind("./qd:hotend", self.__namespaces)
    ...
    def _getSettingsDictForNode(cls, node) -> Tuple[Dict[str,  Any], Dict[str, Any]]:
        ...
        # Fetch settings in the "qd" namespace
        um_settings = node.iterfind("./qd:setting", cls.__namespaces)
        for um_setting_entry in um_settings:
            ...
            if setting_key in cls.__material_settings_setting_map:
                if setting_key == "processing temperature graph":  # This setting has no setting text but subtags.
                    graph_nodes = um_setting_entry.iterfind("./qd:point", cls.__namespaces)
                    graph_points = []
                    for graph_node in graph_nodes:
                        flow = float(graph_node.get("flow"))
                        temperature = float(graph_node.get("temperature"))
        ...
        # Fetch settings in the "qidi" namespace
        qidi_settings = node.iterfind("./qidi:setting", cls.__namespaces)
        for qidi_setting_entry in qidi_settings:
            value = qidi_setting_entry.text
            if value.lower() == "yes":
                value = True
            elif value.lower() == "no":
                value = False
            key = qidi_setting_entry.get("key")

            # QIDI settings are all mapped
    ...
    def deserializeMetadata(cls, serialized: str, container_id: str) -> List[Dict[str, Any]]:
        ...
        for entry in data.iterfind("./qd:metadata/*", cls.__namespaces):
            tag_name = _tag_without_namespace(entry)

            if tag_name == "name":
                brand = entry.find("./qd:brand", cls.__namespaces)
                material = entry.find("./qd:material", cls.__namespaces)
                color = entry.find("./qd:color", cls.__namespaces)
                label = entry.find("./qd:label", cls.__namespaces)
        ...
        property_values = {}
        properties = data.iterfind("./qd:properties/*", cls.__namespaces)
        for entry in properties:
            tag_name = _tag_without_namespace(entry)
            property_values[tag_name] = entry.text
        ...
        compatible_entries = data.iterfind("./qd:settings/qd:setting[@key='hardware compatible']", cls.__namespaces)
        try:
        ...
        for machine in data.iterfind("./qd:settings/qd:machine", cls.__namespaces):
            machine_compatibility = common_compatibility
            for entry in machine.iterfind("./qd:setting[@key='hardware compatible']", cls.__namespaces):
                if entry.text is not None:
                    machine_compatibility = cls._parseCompatibleValue(entry.text)

            for identifier in machine.iterfind("./qd:machine_identifier", cls.__namespaces):
                machine_id_list = product_id_map.get(identifier.get("product", ""), [])
                if not machine_id_list:
                ...
                for machine_id in machine_id_list:
                    ...
                    buildplates = machine.iterfind("./qd:buildplate", cls.__namespaces)
                    ...
                    for buildplate in buildplates:
                        ...
                        settings = buildplate.iterfind("./qd:setting", cls.__namespaces)
                    ...
                    for hotend in machine.iterfind("./qd:hotend", cls.__namespaces):
                        hotend_name = hotend.get("id")
                        if hotend_name is None:
                            continue

                        hotend_compatibility = machine_compatibility
                        for entry in hotend.iterfind("./qd:setting[@key='hardware compatible']", cls.__namespaces):
                            if entry.text is not None:
                        ...
                        #
                        buildplates = hotend.iterfind("./qd:buildplate", cls.__namespaces)
                        for buildplate in buildplates:
    ...
    def _addSettingElement(self, builder, instance):
        ...
        if key in self.__material_settings_setting_map.values():
            ...
            key = QD.Dictionary.findKey(self.__material_settings_setting_map, instance.definition.key)
            tag_name = "setting"
    ...
    _metadata_tags_that_have_qidi_namespace = {"pva_compatible", "breakaway_compatible"}

    # Map XML file setting names to internal names
    __material_settings_setting_map = {
        "print temperature": "default_material_print_temperature",
        "heated bed temperature": "default_material_bed_temperature",
        "heated bed temperature initial layer": "material_bed_temperature_layer_0",
        ...
        "chamber cooling": "chamber_cooling_fan_speed",
        "rapid cooling": "rapid_cooling_fan_speed"
    }  # type: Dict[str, str]
    __unmapped_settings = [
        "hardware compatible",
    ...
    __namespaces = {
        "qd": "http://www.qidi.com/material",
        "qidi": "http://www.qidi.com/qidi"
    }
